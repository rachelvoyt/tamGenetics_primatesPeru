...
```{r}
indexPrimers_allCombos <- rbind(indexPrimers_setA,
                                indexPrimers_setB,
                                indexPrimers_setC,
                                indexPrimers_setD) %>%
  distinct() # confirm 384 combos
```
...
indexPlates_sourceWells <- data.frame(
  name = c(indexPrimers_allCombos[, c("i5")], indexPrimers_allCombos[, c("i7")])
) %>%
...
need 10uM each primer; so need to dilute 100 uM as:

50 ul total combo
2.5 each primer + 27.5 water each primer >> do 100 ul total instead
...
tamGenetics_xtnTransfer_p5ab <- rbind(tamGenetics_nonPlateHairXtns_pcr1Plate5ab,
  ...
  mutate(
    ...
    ot2set = case_when(
      ...
      as.numeric(transferID) > 48 ~ "set3_tubes49to54"
    ),
    reagent = xtnID,
...
tamGenetics_mmTransfer_pcr1_p5ab <- data.frame(
  ...
  source_labware = nestPlate,
  source_slot = 6,
  source_well = c(rep("A1", 7), rep("A2", 7)),
  asp_height = 1,
  dest_labware = nestPlate,
...
sink("./robotProtocols/inputFiles/tamGenetics_pcr1Plate5ab_string.txt")
...
# csv file
write.csv(tamGenetics_pcr1Plate5ab, "./robotProtocols/inputFiles/tamGenetics_pcr1Plate5ab.csv", row.names = F)
```
