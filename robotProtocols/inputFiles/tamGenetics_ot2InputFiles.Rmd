...
```{r}
# Labware
xtnPlate <- "denvillewithaxygenbase_96_wellplate_200ul"
pcrPlate <- "denvillewithaxygenbase_96_wellplate_200ul"
...
primers_toDilutionPlate <- dilutionPlate %>%
  ...
  mutate(dest_slot = 11,
         ...
         dest_labware = "denvillewithaxygenbase_96_wellplate_200ul",
         transfer_volume = 10) %>%
...
bfh_sourceWells_fecalTubes25to32 <- bfh_xtns %>%
  ...
  mutate(
    source_well.t = wellValues_tuberack[1:length(xtnID)],
    fecalSet = "set4.1_fecalTubes25to32"
    ) %>%
  select(c("xtnID", "source_well.t", "fecalSet"))
...
bfh_transferInfo_pp2 <- bfh_xtns %>%
  ...
  mutate(
    ot2set = case_when(
      (str_detect(xtnID, "blood") & str_detect(xtnID, "p1")) ~ "set1.2_xtnPlate1",
      (str_detect(xtnID, "blood") & str_detect(xtnID, "p2")) ~ "set2.1_xtnPlate2",
      str_detect(xtnID, "blood_t") ~ "set2.2_bloodTubes",
      str_detect(sampleType, "fecal") ~ fecalSet,
      str_detect(xtnID, "hair") & str_detect(xtnID, "p3") ~ "set4.2_xtnPlate3",
      str_detect(xtnID, "hair") & str_detect(xtnID, "p4") ~ "set5.1_xtnPlate4",
      str_detect(xtnID, "hair_t") ~ "set5.2_hairTubes",
    ),
    reagent = xtnID,
...
bfh_pcr1_mmTransfer <- data.frame(
  ot2set = rep("set1.1_water.mm", 24),
  reagent = "mastermix",
  source_labware = pcrPlate,
  source_slot = 6,
  source_well = c(rep("B1", 12), rep("C1", 12)),
  asp_height = 1,
  dest_labware = pcrPlate,
...
```{r}
bfh_pcr1_fullTransferInput <- rbind(bfh_pcr1_waterTransfer,
                           bfh_pcr1_mmTransfer,
...
```{r}
# xtn list plate 1
tamGenetics_xtnsBlood_p1 <- xtnsAll_updated %>%
  filter(lysisLoc == "p1") %>%
  filter(eltn == "e1") %>%
  ...
  select(c("transferID", "sampleType_animalID", "sampleType", "xtnID", "xtnRemaining")) %>%
  # replace "SKIP" xtns
  merge(., xtnsRemaining_byAnimalID, by = "sampleType_animalID", all.x = T) %>%
  mutate(
    xtnID = case_when(
  ...
  select(c("transferID", "sampleType_animalID", "xtnID")) %>%
  arrange(transferID)
  
# assign source wells
tamGenetics_sourceWells_bloodTubes <- tamGenetics_xtnsBlood_p1 %>%
  filter(str_detect(xtnID, "blood_t")) %>%
  mutate(
    source_well.t = wellValues_tuberack[1:length(xtnID)]
  )

tamGenetics_xtnTransfer_p1 <- tamGenetics_xtnsBlood_p1 %>%
  merge(., tamGenetics_sourceWells_bloodTubes[, c("xtnID", "source_well.t")], by = "xtnID", all.x = T) %>%
  arrange(as.numeric(transferID)) %>%
  mutate(
    ot2set = case_when(
      str_detect(xtnID, "p") & str_detect(xtnID, "e1") ~ "set1.2_xtnPlate1",
      str_detect(xtnID, "blood_t") ~ "set1.3_xtnPlate1.tubes"
      ),
    reagent = xtnID,
    source_labware = case_when(
      str_detect(xtnID, "p") ~ pcrPlate,
