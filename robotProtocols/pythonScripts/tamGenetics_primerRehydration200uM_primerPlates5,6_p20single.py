...
def run(ctx):
    ...
    for line in transfer_info:
        ...
        dest = ctx.loaded_labwares[
            int(d_slot)].wells_by_name()[parse_well(d_well)].bottom(float(30))
        if tip_reuse == 'always':
            pick_up()
        pip.transfer(float(vol),
                     source,
                     dest,
                     blow_out = True,
