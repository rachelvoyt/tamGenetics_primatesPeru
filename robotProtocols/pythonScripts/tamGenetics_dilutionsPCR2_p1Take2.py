...
metadata = {
    'apiLevel': '2.8',
    'protocolName': 'tamGenetics_dilutionsPCR2_p1Take2',
    'author': 'Rachel Voyt',
...
def run(protocol):
    ...
    # transfers for SET 1: water to pcr1 + mix + 0.5x diluted pcr1a to dilution plate + water to dilution plate + mix + combined a/b dilution to pcr2 plate
    for line in transfer_info:
        if line[0].startswith('set2'):
            ...
            pcr1Plate = lw_pcr1Plate.wells_by_name()[parse_well(d_well)].bottom(1)
    ...
    # transfers for SET 2: index primers to pcr2 plate
    for line in transfer_info:
        if line[0].startswith('set2'):
            _, _, _, _, s_well, _, _, _, d_well, _, vol = line[:11]
