...
Instead, we need to create this based on the results from GTseq_SeqTest.pl.
This script outputs files for each sample that contain three columns, which represent counts of the following:
...
However, we also need allele frequency metrics per locus.
...
## 4.2 On-target reads per sample
...
```{r}
summary(genoRate$genoRate_propDiff, na.rm = T)
```
...
The GTseq pipeline, however, doesn't have a script that provides metrics per locus from what I could tell.
Instead, we need to create this based on the results from GTseq_SeqTest.pl

The GTseq_SeqTest.pl script outputs files for each sample that contain three columns, which represent counts of each... - primer - probe - primer-probe combination
