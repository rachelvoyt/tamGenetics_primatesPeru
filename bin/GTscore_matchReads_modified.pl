...
#--------------------------------------------------------------------------------
# NOTE: RV modified to include inDir (for fastq files) and outDir options; aside from adding options in setup, added lines 261-263 to direct output to outDir.
...
GetOptions('p=s' => \$primerProbeFile, 'files=s' => \$sequenceFiles, 'inDir=s' => \$inDir, 'outDir=s' => \$outDir, 'prefix=s' =>\$prefix, 'matchType=s' =>\$matchType);
...
if ($outDir){
	$outfile=$outDir."/".$prefix."matchedReads_".$matchType."Aligned.txt";
}
