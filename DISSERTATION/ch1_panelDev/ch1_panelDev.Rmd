...
# loci lists
run4_lwedLoci <- tamRun4_primerProbeFile %>%
  filter(!str_detect(Locus, "SIMP"))

run4_simpLoci <- tamRun4_primerProbeFile %>%
  filter(!str_detect(Locus, "LWED"))
```
...
Function to sum the read counts per allele for each locus, using package gsubfn
...
run4_dsGenoCompare_ds6 <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 6,
  n_iterations = 1000)

run4_dsGenoCompare_ds10 <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 10,
  n_iterations = 1000)

run4_dsGenoCompare_ds15 <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 15,
  n_iterations = 1000)

run4_dsGenoCompare_ds20 <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 20,
  n_iterations = 1000)

run4_dsGenoCompare_ds30 <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 30,
  n_iterations = 1000)

run4_dsGenoCompare_ds50 <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 50,
  n_iterations = 1000)

run4_dsGenoCompare_ds100 <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 100,
  n_iterations = 1000)
...
```{r}
run4_dsGenoCompare_dsAll <- rbind(
  run4_dsGenoCompare_ds6 %>% mutate(ds = "6x"),
  run4_dsGenoCompare_ds10 %>% mutate(ds = "10x"),
  run4_dsGenoCompare_ds15 %>% mutate(ds = "15x"),
  run4_dsGenoCompare_ds20 %>% mutate(ds = "20x"),
  run4_dsGenoCompare_ds30 %>% mutate(ds = "30x"),
  run4_dsGenoCompare_ds50 %>% mutate(ds = "50x"),
  run4_dsGenoCompare_ds100 %>% mutate(ds = "100x")
)

run4_dsGenoCompare_dsAll$ds <- factor(run4_dsGenoCompare_dsAll$ds, levels = c("6x", "10x", "15x", "20x", "30x", "50x", "100x"))

ggplot(run4_dsGenoCompare_dsAll, aes(x = factor(ds), y = max_propMatch)) +
  geom_boxplot()

run4_dsGenoCompare_dsAll %>%
  group_by(ds, locus) %>%
  summarise(
