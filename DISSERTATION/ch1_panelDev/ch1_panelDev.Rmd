...
Includes genos for gtseq (genotyper_v2)
...
ahhh I've messed up and put geno success stuff here...

```{r}
run4_gtseq_sampleSum %>%
  group_by(sampleType) %>%
  summarise(
    mean_genoSuccess = mean(genoSuccess, na.rm = T),
    sd_genoSuccess = sd(genoSuccess, na.rm = T)
  )
```

#### bySample OLD STUFF BELOW
...
```{r}
run4_lwed_genos_temp <- run4_genos_gtseq %>%
  # 1. subset to lwed samples only
  select(run4_lwedSamples$sampleID_ill) %>%
  # 2. remove simp-specific loci
  filter(!str_detect(rownames(.), "SIMP")) %>%
...
run4_simp_genos_temp <- run4_genos_gtseq %>%
  # 1. subset to simp samples only
  select(run4_simpSamples$sampleID_ill) %>%
  # 2. remove simp-specific loci
  filter(!str_detect(rownames(.), "LWED")) %>%
  # 3. remove SNPs with >50% missing data
  select(where(~sum(!is.na(.x))/length(.x) >= 0.5)) %>%
...
One to remove - fecal sample tamRun4.053/animalID 210 (in the lwed subset)

```{r}
run4_speciesChecks_lwed <- assignSpecies(
  genoFile = run4_lwed_genos_temp,
  mdFile = run4_md,
  sampleID_colName = "sampleID_ill") %>%
  filter(sampleID %in% run4_posSamples$sampleID_ill)

run4_speciesChecks_simp <- assignSpecies(
  genoFile = run4_simp_genos_temp,
  mdFile = run4_md,
  sampleID_colName = "sampleID_ill") %>%
  filter(sampleID %in% run4_posSamples$sampleID_ill)

# tamRun4.053 to remove
run4_toRemove_speciesChecks_lwed <- run4_speciesChecks_lwed %>%
  filter(mdMatch == F) %>%
  select(sampleID) %>%
  pull()

# none to remove
run4_toRemove_speciesChecks_simp <- run4_speciesChecks_simp %>%
  filter(mdMatch == F) %>%
  select(sampleID) %>%
  pull()
```
...
tamRun4.022 (lwed set) & tamRun4.027 (simp set) both had majority mismatch; both are blood samples though so nbd

```{r}
run4_sexChecks_lwed <- assignSex(run4_lwed_genos_temp, run4_md, "sampleID_ill", exclude_nonTargetSp = "yes") %>%
...
run4_sexChecks_simp <- assignSex(run4_simp_genos_temp, run4_md, "sampleID_ill", exclude_nonTargetSp = "yes") %>%
  #filter(mdMatch == FALSE) %>%
  mutate(
    majSex = case_when(
      propF > propM ~ "F",
...
```{r}
run4_genos_clean_lwed <- run4_lwed_genos_temp %>%
  select(!all_of(run4_toRemove_speciesChecks_lwed))

run4_genos_clean_simp <- run4_simp_genos_temp
```

### Clean summaries

```{r}
temp1 <- run4_genos_clean_lwed %>%
  t() %>%
  as.data.frame() %>%
  mutate(possibleGenos = ncol(.),
         totalGenos = rowSums(!is.na(.))-1,
         genoSuccess = totalGenos / possibleGenos) %>%
  rownames_to_column("sampleID") %>%
  select(sampleID, genoSuccess)
temp2 <- run4_genos_clean_simp %>%
  t() %>%
  as.data.frame() %>%
  mutate(possibleGenos = ncol(.),
         totalGenos = rowSums(!is.na(.))-1,
...
run4_gtseq_sampleSum_clean <- rbind(temp1, temp2) %>%
  merge(., sampleRef[, c("sampleID", "animalID", "sampleType")], by = "sampleID") %>%
  relocate(animalID) %>%
  select(-sampleID)

table(run4_gtseq_sampleSum_clean$sampleType)
```
...
# none are normally distributed >> run wilcox vs. t-test
...
run4_stat.test_genoSuccess_bySampleType <- run4_gtseq_genoSuccess_bySample %>%
  rstatix::pairwise_wilcox_test(genoSuccess ~ sampleType,
                                paired = T,
                                p.adjust.method = "holm") %>%
  rstatix::add_xy_position(x = "sampleType")
run4_stat.test_genoSuccess_bySampleType

run4_stat.test_genoSuccess_bySampleType$groups
```
...
run4_gtseq_sampleSum %>%
  ...
  stat_pvalue_manual(run4_stat.test_genoSuccess_bySampleType, label = "p.adj.signif", tip.length = 0.01, inherit.aes = F, bracket.nudge.y = 100, step.increase = 0.07) +
...
run4_spAssignments_gtseq %>%
  ...
  as.data.frame() %>%
  mutate(propMatch = mdMatch / (mdMatch + mdMismatch)) %>%
  ggplot(aes(x = sampleType, y = propMatch, color = sampleType)) +
  geom_bar(stat = "identity", aes(color = sampleType)) +
  theme_Publication_rv() +
  scale_color_manual(values = my_colors_med) +
  labs(
    x = element_blank(),
    y = "% match"
  )
...
# all sex genos matched metadata - 
table(run4_sexAssignments_gtseq$mdMatch)
60/68 # 88.2
# majority sex genos matched metadata - 97.0%
table(run4_sexAssignments_gtseq$mdMatch_sexMajority)
64/68 # 94.1
...
run4_sexAssignments_gtseq %>%
  ...
  as.data.frame() %>%
  mutate(
    totalSamples = mdMatch + mdMismatch,
    propMatch = mdMatch/totalSamples,
    propMatch_majority = mdMatch_maj/totalSamples
  ) %>%
  ggplot(aes(x = sampleType, y = propMatch_majority, color = sampleType)) +
...
Remove those with < 50% geno success AND tamRun4.053 (mismatch species assignment)

```{r}
run4_genos_gtseq_clean <- run4_genos_gtseq %>%
  select(run4_gtseq_genoSuccess_bySample_over50$sampleID) %>%
  select(-tamRun4.053)
```
...
```{r}
run4_lwed_genos_forGenind <- run4_genos_gtseq_clean %>%
  # indid for lwed only
  filter(!str_detect(rownames(.), "SIMP|SPECIES|SEX")) %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column("sampleID") %>%
  merge(., sampleRef[, c("sampleID", "sampleType")], by = "sampleID") %>%
  relocate(sampleType, .after = sampleID)
run4_simp_genos_forGenind <- run4_genos_gtseq_clean %>%
  # indid for simp only
  filter(!str_detect(rownames(.), "LWED|SPECIES|SEX")) %>%
  t() %>%
  as.data.frame() %>%
  rownames_to_column("sampleID") %>%
  merge(., sampleRef[, c("sampleID", "sampleType")], by = "sampleID") %>%
...
# by sample type
run4_lwed_genos_forGenind_b <- run4_lwed_genos_forGenind %>%
  filter(sampleType == "blood") %>% select(-sampleType)
run4_lwed_genos_forGenind_f <- run4_lwed_genos_forGenind %>%
  filter(sampleType == "fecal") %>% select(-sampleType)
run4_lwed_genos_forGenind_h <- run4_lwed_genos_forGenind %>%
  filter(sampleType == "hair") %>% select(-sampleType)

run4_simp_genos_forGenind_b <- run4_simp_genos_forGenind %>%
  filter(sampleType == "blood") %>% select(-sampleType)
run4_simp_genos_forGenind_f <- run4_simp_genos_forGenind %>%
  filter(sampleType == "fecal") %>% select(-sampleType)
run4_simp_genos_forGenind_h <- run4_simp_genos_forGenind %>%
  filter(sampleType == "hair") %>% select(-sampleType)

# genind
run4_lwed_genind_blood <- adegenet::df2genind(X = run4_lwed_genos_forGenind_b[,c(2:155)],
                                  sep = ",",
                                  ind.names = run4_lwed_genos_forGenind_b$sampleID,
                                  NA.char = "NA",
                                  ploidy = 2,
                                  type = "codom")
run4_lwed_genind_fecal <- adegenet::df2genind(X = run4_lwed_genos_forGenind_f[,c(2:155)],
                                  sep = ",",
                                  ind.names = run4_lwed_genos_forGenind_f$sampleID,
                                  NA.char = "NA",
                                  ploidy = 2,
                                  type = "codom")
run4_lwed_genind_hair <- adegenet::df2genind(X = run4_lwed_genos_forGenind_h[,c(2:155)],
                                  sep = ",",
                                  ind.names = run4_lwed_genos_forGenind_h$sampleID,
                                  NA.char = "NA",
                                  ploidy = 2,
                                  type = "codom")

run4_simp_genind_blood <- adegenet::df2genind(X = run4_simp_genos_forGenind_b[,c(2:155)],
                                  sep = ",",
                                  ind.names = run4_simp_genos_forGenind_b$sampleID,
                                  NA.char = "NA",
                                  ploidy = 2,
                                  type = "codom")
run4_simp_genind_fecal <- adegenet::df2genind(X = run4_simp_genos_forGenind_f[,c(2:155)],
                                  sep = ",",
                                  ind.names = run4_simp_genos_forGenind_f$sampleID,
                                  NA.char = "NA",
                                  ploidy = 2,
                                  type = "codom")
run4_simp_genind_hair <- adegenet::df2genind(X = run4_simp_genos_forGenind_h[,c(2:155)],
                                  sep = ",",
                                  ind.names = run4_simp_genos_forGenind_h$sampleID,
                                  NA.char = "NA",
                                  ploidy = 2,
                                  type = "codom")
```
...
```{r}
run4_lwed_pidPerm_blood <- PopGenUtils::pid_permute(obj = run4_lwed_genind_blood, nrep = 1000)
run4_lwed_pidPerm_fecal <- PopGenUtils::pid_permute(obj = run4_lwed_genind_fecal, nrep = 1000)
run4_lwed_pidPerm_hair <- PopGenUtils::pid_permute(obj = run4_lwed_genind_hair, nrep = 1000)

run4_simp_pidPerm_blood <- PopGenUtils::pid_permute(obj = run4_simp_genind_blood, nrep = 1000)
run4_simp_pidPerm_fecal <- PopGenUtils::pid_permute(obj = run4_simp_genind_fecal, nrep = 1000)
run4_simp_pidPerm_hair <- PopGenUtils::pid_permute(obj = run4_simp_genind_hair, nrep = 1000)
```
...
```{r}
# LWED
run4_lwed_pidPerm_data <- rbind(
  run4_lwed_pidPerm_blood$median_values %>% mutate(sampleType = "blood"),
  run4_lwed_pidPerm_fecal$median_values %>% mutate(sampleType = "fecal"),
  run4_lwed_pidPerm_hair$median_values %>% mutate(sampleType = "hair"))

## PID meets 0.0001 threshold at 11 (blood) or 12 loci (fecal/hair)
run4_lwed_pid.thresh <- run4_lwed_pidPerm_data %>%
  group_by(sampleType) %>%
  filter(PID < 0.0001) %>%
  summarise(
    nLoci = loci[1], # First locus where PID meets threshold
    pidThresh = PID[1]
  )
  
# original code for above (w/o splitting by sampleType): as.numeric(run4_lwed_pidPerm_data[which(run4_lwed_pidPerm_data$PID < 0.0001, arr.ind=TRUE)[1],][[1]])

## PIDsibs meets 0.0001 threshold at 21 (blood), 22 (fecal), and 23 (hair) loci
run4_lwed_pidSibs.thresh <- run4_lwed_pidPerm_data %>%
  group_by(sampleType) %>%
  filter(PIDsibs < 0.0001) %>%
  summarise(
    nLoci = loci[1], # First locus where PID meets threshold
    pidThresh = PIDsibs[1]
  )
...
# SIMP
run4_simp_pidPerm_data <- rbind(
  run4_simp_pidPerm_blood$median_values %>% mutate(sampleType = "blood"),
  run4_simp_pidPerm_fecal$median_values %>% mutate(sampleType = "fecal"),
  run4_simp_pidPerm_hair$median_values %>% mutate(sampleType = "hair"))

## PID meets 0.0001 threshold at 11 (blood/fecal) or 12 loci (hair)
run4_simp_pid.thresh <- run4_simp_pidPerm_data %>%
  group_by(sampleType) %>%
  filter(PID < 0.0001) %>%
  summarise(
    nLoci = loci[1],
...
## PIDsibs meets 0.0001 threshold at 21 (blood/fecal) and 23 (hair) loci
run4_simp_pidSibs.thresh <- run4_simp_pidPerm_data %>%
  group_by(sampleType) %>%
  filter(PIDsibs < 0.0001) %>%
  summarise(
    nLoci = loci[1], # First locus where PID meets threshold
...
```{r}
# get animalIDs w/all sampleTypes over 50% genoSuccess
run4_genoCompare_sampleTypes_sampleList <- run4_md %>%
  filter(animalID %in% run4_gtseq_genoSuccess_bySample_setsOver50$animalID) %>%
  filter(animalID != 210)

# end up w/12 animalIDs
length(unique(run4_genoCompare_sampleTypes_sampleList$animalID)) # 12 sets

# compare genos called b/t sampleTypes
...
run4_genoCompare_sampleTypes_gtseq %>%
  nrow() # 2496 records w/o filtering
run4_genoCompare_sampleTypes_gtseq %>%
  na.omit() %>%
  nrow() # 1363 if only only keep records where genos for all sampleTypes

test <- run4_genoCompare_sampleTypes_gtseq %>%
  select(locus) %>%
  distinct(locus) %>%
  merge(., run4_genoCompare_sampleTypes_gtseq[, c("animalID", "locus", "bfMatch", "bhMatch", "fhMatch")], by = "locus", all.x = T) %>%
  na.omit() %>%
  group_by(locus) %>%
  summarise(count = n()) %>%
  filter(count >= 10)
...
Not going to filter to loci genotyped across all sampletypes for all indivs; going on a pair basis
...
run4_genoCompare_sampleTypes_gtseq %>%
  ...
  summarise(
    mean_bfCommon = mean(bfCommon),
    sd_bfCommon = sd(bfCommon),
    mean_bhCommon = mean(bhCommon),
    sd_bhCommon = sd(bhCommon),
    mean_fhCommon = mean(fhCommon),
    sd_fhCommon = sd(fhCommon),
    
    mean_bfMatch = mean(bf_propMatch)*100,
    sd_bfMatch = sd(bf_propMatch)*100,
    mean_bhMatch = mean(bh_propMatch)*100,
    sd_bhMatch = sd(bh_propMatch)*100,
    mean_fhMatch = mean(fh_propMatch)*100,
    sd_fhMatch = sd(fh_propMatch)*100
  )
```
...
run4_genoCompare_sampleTypes_gtseq_sampleSum <- run4_genoCompare_sampleTypes_gtseq %>%
  # retain only loci genotyped at all sampleTypes for at least 10/12 samples
  #filter(locus %in% test$locus) %>%
#  na.omit() %>%
  group_by(animalID) %>%
  summarise(
...
#      str_detect(animalID, "SIMP") ~ genosCommon/175
      ...
      str_detect(animalID, "LWED") ~ bfCommon/nrow(run4_lwedLoci),
      str_detect(animalID, "SIMP") ~ bfCommon/nrow(run4_simpLoci)
    ),
    bh_propCommon = case_when(
#      str_detect(animalID, "LWED") ~ genosCommon/173,
#      str_detect(animalID, "SIMP") ~ genosCommon/175
      
      str_detect(animalID, "LWED") ~ bhCommon/nrow(run4_lwedLoci),
      str_detect(animalID, "SIMP") ~ bhCommon/nrow(run4_simpLoci)
    ),
    fh_propCommon = case_when(
#      str_detect(animalID, "LWED") ~ genosCommon/173,
#      str_detect(animalID, "SIMP") ~ genosCommon/175
      
      str_detect(animalID, "LWED") ~ fhCommon/nrow(run4_lwedLoci),
      str_detect(animalID, "SIMP") ~ fhCommon/nrow(run4_simpLoci)
    )
  )
```
...
stats:
```{r}
run4_genoCompare_sampleTypes_stats.df <- run4_genoCompare_sampleTypes_gtseq_sampleSum %>%
  select(animalID, contains("propMatch")) %>%
  pivot_longer(-animalID,
               names_to = "compType",
               values_to = "propMatch")

temp <- run4_genoCompare_sampleTypes_stats.df %>%
  pivot_wider(id_cols = animalID,
              names_from = compType,
              values_from = propMatch)

# all but fecal vs. hair are normally distributed >> run wilcox vs. t-test anyway
shapiro.test(temp$bf_propMatch) # 0.5282
shapiro.test(temp$bh_propMatch) # 0.1928
shapiro.test(temp$fh_propMatch) # 0.02262

run4_stat.test_genoCompare_sampleTypes <- run4_genoCompare_sampleTypes_stats.df %>%
  rstatix::pairwise_wilcox_test(propMatch ~ compType, paired = T) %>%
  rstatix::add_xy_position(x = "compType")
run4_stat.test_genoCompare_sampleTypes$groups

# no sig diffs b/t groups
run4_stat.test_genoCompare_sampleTypes
```
...
fig1.2 <- run4_genoCompare_sampleTypes_gtseq_sampleSum %>%
  select(animalID, contains("propMatch")) %>%
  ...
  #scale_colour_Publication() +
  theme_Publication_rv() +
  scale_color_manual(values = c("#4C413FFF", "#E75B64FF", "#278B9AFF")) +
  labs(x = "",
...
# not significant; don't add
  #stat_pvalue_manual(run4_stat.test_genoCompare_sampleTypes, label = "p.adj.signif", tip.length = 0.01, inherit.aes = F, bracket.nudge.y = 100, step.increase = 0.07)
...
# read back in
run4_dsGenoCompare_dsAll.arML <- read.csv("./DISSERTATION/ch1_panelDev/02_results/run4_dsGenoCompare_dsAll.arML.csv")
```
...
```{r}
# should be doing non-param test--
run4_stat.test_dsGenoCompare <- run4_dsGenoCompare_dsAll.arML %>%
  filter(!ds %in% c("3x", "6x")) %>%
  select(ds, sampleID, locus, max_propMatch, pipeline) %>%
  group_by(ds, sampleID) %>%
  pairwise_wilcox_test(max_propMatch ~ pipeline, paired = T)
...
run4_genoCompare_sampleTypes_stats.df %>%
  rstatix::pairwise_wilcox_test(propMatch ~ compType, paired = T) %>%
  rstatix::add_xy_position(x = "compType")
...
## data for plot
data_fig1.3 <- run4_dsGenoCompare_dsAll.arML %>%
  ...
  filter(!ds %in% c("3x", "6x")) %>%
  mutate(ds = factor(ds, levels = c("10x", "20x", "30x", "50x", "100x"))) %>%
...
# stats
stats_fig1.3 <- run4_dsGenoCompare_dsAll.arML %>%
  mutate(ds = factor(ds, levels = c("10x", "20x", "30x", "50x", "100x"))) %>%
  filter(pipeline == "gtseq") %>%
  filter(!ds %in% c("3x", "6x")) %>%
  merge(., sampleRef[, c("sampleID", "animalID")], by = "sampleID") %>%
  pivot_wider(id_cols = c(ds, animalID, locus),
              names_from = sampleType,
              values_from = max_propMatch) %>%
  pivot_longer(-c(ds, animalID, locus),
               names_to = "sampleType",
               values_to = "max_propMatch") %>%
  group_by(ds) %>%
  rstatix::pairwise_wilcox_test(
        formula = max_propMatch ~ sampleType,
        paired = T,
        p.adjust.method = "holm") %>%
  rstatix::add_x_position(x = "ds",
                          group = "sampleType",
                          dodge = 0.8)

# plot (couldn't get it to work w/ggplot, but ggpubr seems happy enough)
fig1.3 <- data_fig1.3 %>%
  mutate(mean_maxMatch = mean_maxMatch*100) %>%
  ggboxplot(x = "ds", y = "mean_maxMatch", fill = "sampleType", outlier.size = 0.5) +
  ggpubr::stat_pvalue_manual(stats_fig1.3,
                             label = "p.adj.signif",
                             tip.length = 0.01,
                             y.position = 100,
...
```{r}
# stats
stats_fig1.4 <- run4_dsGenoCompare_dsAll.arML %>%
  mutate(ds = factor(ds, levels = c("3x", "6x", "10x", "20x", "30x", "50x", "100x"))) %>%
  filter(!ds %in% c("3x", "6x")) %>%
  select(ds, pipeline, max_propMatch) %>%
  group_by(ds) %>%
  rstatix::pairwise_wilcox_test(
    max_propMatch ~ pipeline,
    paired = T,
    p.adjust.method = "holm"
  ) %>%
  rstatix::add_x_position(x = "ds",
...
data_fig1.4 <- run4_dsGenoCompare_dsAll.arML %>%
  mutate(ds = factor(ds, levels = c("3x", "6x", "10x", "20x", "30x", "50x", "100x"))) %>%
  mutate(pipeline = case_when(
    pipeline == "gtseq" ~ "Allele-ratio",
    pipeline == "gtscore" ~ "Max-likelihood"
...
fig1.4 <- data_fig1.4 %>%
  ...
  theme_Publication_rv() +
  scale_fill_manual(values = c("#ADB7C0FF", "#ECD89DFF")) +
  labs(x = "Read depth",
       y = "Mean % genotype concordance") +
  guides(fill = guide_legend(title = "Pipeline")) +
  theme(legend.position = "right")+
  ggpubr::stat_pvalue_manual(stats_fig1.4,
                             label = "p.adj.signif",
                             tip.length = 0.01,
                             y.position = 101,
                             bracket.nudge.y = 0.25,
...
effect size: https://rpkgs.datanovia.com/rstatix/reference/wilcox_effsize.html

```{r}
effSize_fig1.4 <- run4_dsGenoCompare_dsAll.arML %>%
  mutate(ds = factor(ds, levels = c("3x", "6x", "10x", "20x", "30x", "50x", "100x"))) %>%
  filter(!ds %in% c("3x", "6x")) %>%
  select(ds, pipeline, max_propMatch) %>%
  group_by(ds) %>%
...
table1.3 <- stats_fig1.4 %>%
  mutate(
    ...
    p.adj = as.character(p.adj),
    p.adj = case_when(
      is.na(p.adj) ~ "< 0.0001",
      .default = p.adj
    )
  ) %>%
  select(ds, statistic, p.adj) %>%
  cbind(., effSize_fig1.4[, c("effsize", "magnitude")]) %>%
...
table1.3 %>%
  gtsave("./DISSERTATION/ch1_panelDev/02_results/00_tablesFigures/table1.3.png")
```
...
fig_1.4 <- data_fig1.4 %>%
  rbind(., data.frame(
    pipeline = c("Max-likelihood", "Max-likelihood"),
    ds = c("3x", "6x"),
    sampleID = NA,
    mean_maxMatch = NA
  )) %>%
...
## Table 1.1 - GTseq studies

```{r}
data_table1.1 <- read.csv("./DISSERTATION/ch1_panelDev/00_data/googleSheets_table1.1_gtseqExamples.csv") %>%
  filter(noninvasive. == "yes") %>%
  select(2:11) %>%
  mutate(conversionRate = finalPanel / primersDesigned) %>%
  mutate(pipeline = case_match(
    pipeline,
    "AR" ~ "Allele-ratio",
    .default = pipeline
  ))
...
# Create the table
table1.1 <- data_table1.1 %>%
  #relocate(primersDesigned, .before = finalPanel) %>%
  select(-group) %>%
  gt() %>%
  cols_label(
    #group = "Group",
    speciesCommon = "Common Name",
    speciesScientific = "Scientific Name",
    sampleTypes_hq = "High-Quality Samples",
    sampleTypes_lq = "Low-Quality Samples",
    pipeline = "Pipeline",
    finalPanel = "Final Panel",
    primersDesigned = "Primers Designed",
    conversionRate = "Conversion Rate",
    publication = "Publication"
  ) %>%
  fmt_integer(
    columns = c(finalPanel, primersDesigned)
  ) %>%
  fmt_percent(
    columns = conversionRate,
    decimals = 1  # Display percentage with one decimal place
  ) %>%
  tab_style(
  ...
  tab_footnote(
    footnote = "Specific type unspecified",
    locations = cells_body(
      columns = sampleTypes_hq,
      rows = sampleTypes_hq %in% c("Biopsies", "Tissue", "Blood, tissue")
    )
  )
...
table1.1 %>%
  gtsave("./DISSERTATION/ch1_panelDev/02_results/00_tablesFigures/table1.1_gtseqExamples.png")
```
...
```{r}
data_tableA.2 <- read.csv("./DISSERTATION/ch1_panelDev/00_data/quip_tamGenetics_extractionsHair_master_columnDefinitions.csv") %>%
  filter(Column.name %in% c("lysisProtocol", "xtnProtocol")) %>%
  select(-Notes) %>%
  filter(Input.values != "omegaTL_b")

tableA.2 <- data_tableA.2 %>%
  mutate(
    Column.name = case_match(
      Column.name,
      "lysisProtocol" ~ "Lysis protocols",
      "xtnProtocol" ~ "Extraction protocols"
    )
  ) %>%
  dplyr::rename("Protocol description" = "Definition") %>%
  gt(groupname_col = "Column.name", rowname_col = "Input.values")
  cols_label(
    #group = "Group",
    speciesCommon = "Common Name",
    speciesScientific = "Scientific Name",
...
tableA.2 %>%
  gtsave("./DISSERTATION/ch1_panelDev/02_results/00_tablesFigures/SI_tableA.2_hairXtns.png")
```
...
# OLD
fig1.3 <- data_fig1.3 %>%
  #mutate(group1 = sampleType) %>%
  ggplot(aes(x = factor(ds), y = mean_maxMatch)) +
...
# OLD below
  ...
  ungroup %>%
  unnest(c(w.test)) %>%
  rstatix::add_x_position(x = "sampleType",
                          group = "ds",
                          dodge = 0.8)
  
   %>%
        rstatix::add_xy_position(x = "sampleType",
                                 comparisons = list(c("blood", "fecal"), c("blood", "hair"), c("fecal", "hair")))
...
str(stats_fig1.3)
  ...
  mutate(
    ...
    xmax = case_when(
      group2 == "blood" ~ xmin + 0.1,
      group2 == "fecal" ~ xmin + 0.3,
      group2 == "hair" ~ xmin + 0.6
    )
  )
...
stat_pvalue_manual(stats_fig1.3,
                     label = "p.adj.signif",
                     tip.length = 0.01,
                     inherit.aes = F,
                     bracket.nudge.y = 100,
