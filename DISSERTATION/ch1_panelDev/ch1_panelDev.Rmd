...
```{r}
# full set
run4_optSamplesLoci_gtscore100x <- optimize_samplesLoci(run4_readCounts_gtscore100x, 0.9)
...
```{r}
# full set
run4_readCounts.opt_gtscore100x <- optimized_genos(run4_readCounts_gtscore100x, n_samples = 50, propSamples = 0.9)
...
1000 iteration version below (gtscore only):
```{r}
# OLD!!!
# 1000 iterations - just keep for now
run4_dsGenoCompare_locusTable <- read.delim("./05_tamRun5/03_run5GTscore/fullSet_LocusTable_singleSNPs.txt", header = TRUE, stringsAsFactors = FALSE)

run4_dsGenoCompare_ds6_1k <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 6,
  n_iterations = 1000)

run4_dsGenoCompare_ds10_1k <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 10,
  n_iterations = 1000)

run4_dsGenoCompare_ds15_1k <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 15,
  n_iterations = 1000)

run4_dsGenoCompare_ds20_1k <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 20,
  n_iterations = 1000)

run4_dsGenoCompare_ds30_1k <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 30,
  n_iterations = 1000)

run4_dsGenoCompare_ds50_1k <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 50,
  n_iterations = 1000)

run4_dsGenoCompare_ds100_1k <- get_dsReadCounts.genoConsistency(
  run4_readCounts.opt_gtscore100x,
  run4_dsGenoCompare_locusTable,
  n_reads = 100,
  n_iterations = 1000)
...
run4_dsGenoCompare_dsAll.ar <- rbind(
  ...
  mutate(
    ...
    ds = factor(ds, levels = c("6x", "10x", "15x", "20x", "30x", "50x", "100x"))
    ) %>%
...
run4_dsGenoCompare_dsAll.ml <- rbind(
  ...
  relocate(pipeline, ds, sampleID, sampleType) %>%
  arrange(ds, sampleID, locus) %>%
  mutate(ttestID = row_number() - 1055,
         ttestID = case_when(
           ttestID < 0 ~ NA,
           .default = ttestID
         )) %>%
...
GTseq (AR):
-   low = 10x = 0.963 +/- 0.0936
-   high = 100x = 0.991 +/- 0.0518

GTscore (ML):
-   low = 6x = 0.923 +/- 0.113
-   10x = 0.963 +/- 0.0943
-   high = 100x = 0.985 +/- 0.0654
...
```{r}
# 10x - p = 0.932
ttest.10x <- t.test(run4_dsGenoCompare_ds10.ar$max_propMatch, run4_dsGenoCompare_ds10.ml$max_propMatch, paired = T, alternative = "two.sided")

# 20x - p = < 2.2e-16
ttest.20x <- t.test(run4_dsGenoCompare_ds20.ar$max_propMatch, run4_dsGenoCompare_ds20.ml$max_propMatch, paired = T, alternative = "two.sided")

# 30x - p < 2.2e-16
ttest.30x <- t.test(run4_dsGenoCompare_ds30.ar$max_propMatch, run4_dsGenoCompare_ds30.ml$max_propMatch, paired = T, alternative = "two.sided")

# 50x - p = 1.537e-09
ttest.50x <- t.test(run4_dsGenoCompare_ds50.ar$max_propMatch, run4_dsGenoCompare_ds50.ml$max_propMatch, paired = T, alternative = "two.sided")

# 100x - p = 1.767e-07
ttest.100x <- t.test(run4_dsGenoCompare_ds100.ar$max_propMatch, run4_dsGenoCompare_ds100.ml$max_propMatch, paired = T, alternative = "two.sided")
```

### Geno class separated
...
test <- run4_dsGenoCompare_dsAll.arML %>%
  ...
  summarise(
    tidy(t.test(max_propMatch ~ pipeline))
  )

View(test %>% filter(p.value < 0.05))
```

## Tables

```{r}
table_ttest_dsGenoCompare <- data.frame(
  t = c(ttest.10x$statistic, ttest.20x$statistic, ttest.30x$statistic, ttest.50x$statistic, ttest.100x$statistic),
  #df = c(ttest.10x$parameter, ttest.20x$parameter, ttest.30x$parameter, ttest.50x$parameter, ttest.100x$parameter),
  p = c(ttest.10x$p.value, ttest.20x$p.value, ttest.30x$p.value, ttest.50x$p.value, ttest.100x$p.value),
  d = c(ttest.10x$estimate, ttest.20x$estimate, ttest.30x$estimate, ttest.50x$estimate, ttest.100x$estimate),
  lci = c(ttest.10x$conf.int[1], ttest.20x$conf.int[1], ttest.30x$conf.int[1], ttest.50x$conf.int[1], ttest.100x$conf.int[1]),
  uci = c(ttest.10x$conf.int[2], ttest.20x$conf.int[2], ttest.30x$conf.int[2], ttest.50x$conf.int[2], ttest.100x$conf.int[2])
) %>%
  round(., 3) %>%
  mutate(depth = c("10x", "20x", "30x", "50x", "100x")) %>%
  relocate(depth) %>%
  mutate(
    p = case_when(
      p < 0.001 ~ "< 0.001***",
    ...
    ci = str_c("[", as.character(lci), ",", as.character(uci), "]")
    ) %>%
  select(-lci, -uci) %>%
...
table_ttest_dsGenoCompare

table_ttest_dsGenoCompare %>%
  gtsave("./DISSERTATION/ch1_panelDev/02_results/00_tablesFigures/table_ttest_dsGenoCompare.png")
```
...
plot2_dsGenoCompare_ar.vs.ml <- run4_dsGenoCompare_dsAll.arML %>%
  ...
  filter(ds != "6x") %>%
  group_by(sampleType, pipeline, ds, locus) %>%
  summarise(
    mean_maxMatch = mean(max_propMatch)
  ) %>%
...
plot3_dsGenoCompare_ar.vs.ml <- run4_dsGenoCompare_dsAll.arML %>%
  ...
  filter(ds != "6x") %>%
  filter(!str_detect(max_genoClass, "/")) %>%
  group_by(max_genoClass, pipeline, ds, locus) %>%
  summarise(
    mean_maxMatch = mean(max_propMatch)
  ) %>%
  ggplot(aes(x = factor(ds), y = mean_maxMatch, fill = pipeline)) +
...
plot4_dsGenoCompare_ar.vs.ml <- run4_dsGenoCompare_dsAll.arML %>%
  ...
  filter(ds != "6x") %>%
  filter(!str_detect(max_genoClass, "/")) %>%
  group_by(pipeline, sampleType, max_genoClass, ds, locus) %>%
  summarise(
    mean_maxMatch = mean(max_propMatch)
  ) %>%
  ggplot(aes(x = factor(ds), y = mean_maxMatch, fill = pipeline)) +
  ...
  labs(x = "Read depth",
       y = "Mean genotype concordance") +
  guides(fill = guide_legend(title = "Pipeline")) +
  facet_grid(cols = vars(max_genoClass), rows = vars(sampleType))
