...
# booo no signif diffs 
test <- run4_dsGenoCompare_dsAll.arML %>%
  mutate(pipeline = case_when(
    pipeline == "gtseq" ~ "Allele-ratio",
    pipeline == "gtscore" ~ "Max-likelihood"
  )) %>%
  ...
  nest_by(ds) %>%
  mutate(w.test = list(rstatix::pairwise_wilcox_test(
    detailed = T,
  ...
  unnest(c(w.test))
test %>%
  select(ds, group1, group2, estimate, statistic, p, p.adj.signif) %>%
  gt()
