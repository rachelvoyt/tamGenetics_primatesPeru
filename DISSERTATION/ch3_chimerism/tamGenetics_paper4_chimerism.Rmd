...
```{bash}
# lwed
cd /home/rachelvoyt/Documents/UT-Grad/Development/repos/tamGenetics_primatesPeru/DISSERTATION/ch3_chimerism/02_results/00_franz/00_lwed
...
vafDev <- (vaf_allReads - 0.5) %>%
  ...
  rowwise() %>%
  mutate(
    b1 = median(c_across(all_of(hairSamples_clean)), na.rm = TRUE),
    b2 = median(c_across(all_of(vaf_bloodReads_sampleList)), na.rm = TRUE),
...
twinChimerism_lwed_summary <- twinChimerism_lwed_corrected %>%
  ...
  summarise(
    n_loci = n(),
    mean_chimProp = round(mean(chimProp, na.rm = T), 2),
    mean_chimProp_c1 = round(mean(chimProp_c1, na.rm = T), 2),
    mean_chimProp_c2 = round(mean(chimProp_c2, na.rm = T), 2),
...
Average number of loci per indiv?
...
```{r}
count_typeI.II_markers <- twinChimerism_allTwins_corrected %>%
  ...
  merge(., sampleRef[, c("animalID", "sex")], by = "animalID") %>%
  mutate(animalID = str_c(animalID, sex, sep = "_")) %>%
  select(-sex) %>%
  distinct() %>%
  relocate(species, twinID) %>%
  arrange(species, as.numeric(twinID))
count_typeI.II_markers

table_count_typeI.II_markers <- count_typeI.II_markers %>%
  ...
  cols_label(
    ...
    twinID = md("**TwinID**"),
    animalID = md("**IndivID**"),
    type1 = md("**Type I markers**"),
    type2 = md("**Type II markers**"),
    total = md("**Total markers**")
  ) %>%
  cols_align(., align = "center", columns = 2:6)

table_count_typeI.II_markers %>%
  gtsave("./DISSERTATION/ch3_chimerism/02_results/00_tablesFigures/table_count_typeI.II_markers.png", vwidth = 550)
```
...
### genoSuccess vs. diff from 50% chimerism
...
figData_genoPerformance.diff50p <- twinChimerism_allTwins_corrected %>%
  ...
  as.data.frame() %>%
  mutate(diff = abs(0.5 - mean_chimProp_c3)) %>%
...
figData_genoPerformance.diff50p %>%
  ggplot(aes(x = gsCheck, y = diff, color = twinID)) +
...
```{r}
temp <- figData_genoPerformance.diff50p %>%
  filter(!gsCheck %in% c("gs<10%", "10<gs<20%"))

twinChimerism_allTwins_corrected %>%
  filter(twinID %in% temp$twinID) %>%
  group_by(animalID) %>%
  summarise(
    n_loci = n(),
    mean_chimProp_c3 = round(mean(chimProp_c3, na.rm = T), 2)
