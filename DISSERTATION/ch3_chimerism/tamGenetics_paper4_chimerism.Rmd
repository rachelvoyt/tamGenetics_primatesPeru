...
genoSuccess <- get_genoSuccess(genos, locusTable, md, "bySample", sampleID_colName = "sampleID", exclude_nonTargetSp = "no")
...
# clean!
genos_temp <- genos %>%
  select(-toRemove_hairDups, -toRemove_bloodDups)
```
...
# 5 Identify twin pairs

## Get likely twins from capData
...
View(capData_2009to2023 %>% filter(animalID %in% c(161, 162, 163)))
View(capData_2009to2023 %>% filter(animalID %in% c(222, 223, 224)))
```
...
```{r}
bloodReads_allTwins <- twinPairs_run5_l %>%
...
vafDev <- (vaf_allReads - 0.5) %>%
  ...
  rowwise() %>%
  mutate(
    b1 = median(c_across(hairSamples_clean), na.rm = TRUE),
