...
### Clean step #5: General cleanup

Mostly just replacing "NULL", "UNK", etc. with NA values, but also:

-   change rowID 1 ageClass to adult (found that was not in fact a
juv as listed in capData)

```{r}
capData_byIndiv_v5 <- read.csv("./DISSERTATION/ch2_demographics/01_dataOrganization/02_dataCleaning/captureData_byIndividual_v5.csv")

table(capData_byIndiv_v5$ageClass)
table(capData_byIndiv_v5$ageClass_classifier)

na_list <- c("NULL", "unk", "UNK")

capData_byIndiv_v6 <- capData_byIndiv_v5 %>%
  # fix ageClass/classifier variables
  mutate(
    ageClass_classifier = case_when(
      ageClass == "old_adult" ~ "adult_old",
      ageClass == "Young" ~ "adult_young",
      .default = ageClass_classifier
    ),
    ageClass = case_when(
      ageClass %in% na_list ~ NA,
      ageClass == "old_adult" ~ "adult",
...
table(capData_byIndiv_v6$ageClass)
table(capData_byIndiv_v6$ageClass_classifier)
```

#### EXPORT

```{r}
write.csv(capData_byIndiv_v6, "./DISSERTATION/ch2_demographics/01_dataOrganization/02_dataCleaning/captureData_byIndividual_v6.csv", row.names = F)
```
