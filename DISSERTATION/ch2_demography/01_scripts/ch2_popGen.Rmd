...
lwed_genos_rel <- lwed_genos_hf %>%
  select(-pop) %>%
  rownames_to_column("sampleID") %>%
  mutate(across(-sampleID, ~ str_split(.x, ""))) %>%
  unnest_wider(-sampleID, names_sep = ".") %>%
  column_to_rownames("sampleID")

## get geno data for all indivs present in each year
lwed_genos_forRelated <- get_genos_perYear(
  lwed_genos_rel,
  lwed_popData_perYear_allIndivs,
  sampleRef = sampleRef,
  forWhat = "related")
View(lwed_genos_forRelated$`2015`$genoData)

str(simp_genos_forRelated)
```
...
simp_genos_forRelated <- get_genos_perYear(
  simp_genos_rel,
  simp_popData_perYear_allIndivs,
  sampleRef = sampleRef,
  forWhat = "related")
View(simp_genos_forRelated$`2015`$genoData)
