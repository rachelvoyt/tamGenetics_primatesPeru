...
**Create anonymized groupNames**

groupName_anon is numbered by earliest captureYear, then alphabetical

```{r}
groupRef_2009to2023 <- capData_2009to2023 %>%
  select(species, groupName, captureYear) %>%
  # remove loners & unk
  filter(!groupName %in% c("Loners", "UNK")) %>%
  arrange(species, captureYear, groupName) %>%
  group_by(species, groupName) %>%
  slice_min(as.numeric(captureYear)) %>%
  ungroup() %>%
  distinct() %>%
  mutate(groupName_caps = toupper(groupName)) %>%
  arrange(species, captureYear, groupName_caps) %>%
  mutate(
    temp = case_when(
      species == "SIMP" ~ row_number() - 17,
      .default = row_number()
    ),
    temp = sprintf("%02d", temp),
    groupName_anon = case_when(
      species == "LWED" ~ str_c("LW.", temp),
      .default = str_c("TS.", temp)
      )
    ) %>%
  select(-groupName_caps, -captureYear) %>%
  mutate(groupNum = as.numeric(temp)) %>%
  # add loners back in for both lwed & simp
  rbind(., data.frame(species = c("LWED", "SIMP"),
                      groupName = c("Loners", "Loners"),
                      temp = NA,
                      groupName_anon = c("LW.Loners", "TS.Loners"),
                      groupNum = NA))

groupRef_2009to2019 <- capData_2009to2019 %>%
  select(species, groupName, captureYear) %>%
  # remove loners & unk
  filter(!groupName %in% c("Loners", "UNK")) %>%
  arrange(species, captureYear, groupName) %>%
...
For each animalID, the presenceMatrix provides species, sex, captureYear_first, & captureYear_last -- based on this info, also included are columns for each year of the study listing all animalIDs present that year. Indivs are assumed to be present in the population if the year falls between their first and last recorded captureYear, even if they weren't necessarily captured in those intervening years.

### groupMembership

First going to sort out group membership for each indiv in each year of the study -- for cases where indivs weren't necessarily captured, but are known to still be present in the pop (i.e., in years b/t first and last recorded capture), group membership is assumed to be that which was last recorded for that individual. 

```{r}
groupMembership_2009to2023 <- capData_2009to2023 %>%
  ...
  pivot_longer(-animalID,
               ...
               values_to = "groupName") %>%
  arrange(animalID, captureYear) %>%
  # fill in (inferred) groupNames for intervening years in which indivs weren't captured
...
Now combining groupMembership above w/some extra metadata to create presenceMatrix:
...
presenceMatrix_2009to2023 <- capData_2009to2023 %>%
  select(species, animalID, sex, captureYear) %>%
  arrange(animalID, captureYear) %>%
  group_by(animalID) %>%
  slice(c(1, n())) %>%
  ungroup() %>%
  mutate(
    id = janitor::make_clean_names(animalID, use_make_names = F),
    id = case_when(
      str_detect(id, "_2") ~ "captureYear_last",
      .default = "captureYear_first"
      )
    ) %>%
  pivot_wider(id_cols = c(animalID, species, sex),
              names_from = id,
              values_from = captureYear) %>%
  merge(., groupMembership_2009to2023, by = "animalID") %>%
...
# 2009 to 2019
presenceMatrix_2009to2019 <- presenceMatrix_2009to2023 %>%
  select(species, animalID, sex, birthYear_est, captureYear_first, captureYear_last, as.character(2009:2019)) %>%
  filter(captureYear_first <= 2019)
```

**Total indivs present per year**

```{r}
totalIndivs_perYear_lwed <- presenceMatrix_2009to2023 %>%
  filter(species == "LWED") %>%
  select(contains("20")) %>%
  t() %>%
  as.data.frame() %>%
  mutate(totalIndivs = rowSums(!is.na(.))) %>%
  rownames_to_column("year") %>%
  select(year, totalIndivs)

totalIndivs_perYear_simp <- presenceMatrix_2009to2023 %>%
  filter(species == "SIMP") %>%
  select(contains("20")) %>%
  t() %>%
  as.data.frame() %>%
...
```{r}
groupComps_temp <- presenceMatrix_2009to2019 %>%
  # keep first capGroup for the one indiv w/2 groups in one year
  mutate(`2010` = case_when(
    `2010` == "Perro6_Angels" ~ "Perro6",
    .default = `2010`
  )) %>%
  select(-captureYear_first, -captureYear_last) %>%
  pivot_longer(-c(species, animalID, sex, birthYear_est),
               names_to = "captureYear",
               values_to = "groupName") %>%
  filter(!is.na(groupName)) %>%
  mutate(
    sex = case_when(
      birthYear_est == captureYear ~ str_c("J", sex),
      .default = str_c("A", sex)
    )
  ) 

groupComps <- groupComps_temp %>%
  group_by(species, groupName, captureYear) %>%
  summarise(
    AF = sum(sex == "AF"),
    AM = sum(sex == "AM"),
    JF = sum(sex == "JF"),
  ...
  mutate(
    totalAdults = AF + AM,
    totalIndivs = AF + AM + JF + JM
  ) %>%
  # add groupName_anon
  merge(., groupRef_2009to2019[, c("species", "groupName", "groupName_anon")], by = c("species", "groupName"))
```
...
# grp comp all indivs
View(groupComps %>%
  # remove grps w/only 1 indiv
    filter(totalIndivs > 1) %>%
  group_by(species) %>%
  summarise(
    minTotal = min(totalIndivs),
    maxTotal = max(totalIndivs),
    meanTotal = mean(totalIndivs),
    sdTotal = sd(totalIndivs),
    minAdults = min(totalAdults),
    maxAdults = max(totalAdults),
    meanAdults = mean(totalAdults),
    sdAdults = sd(totalAdults),
    minF = min(AF),
    maxF = max(AF),
    meanF = mean(AF),
    sdF = sd(AF),
...
# grp comp adults only
View(groupComps %>%
  filter(!groupName %in% c("Loners", "Angels")) %>%
  group_by(species) %>%
  summarise(
    minAdults = min(totalAdults),
    maxAdults = max(totalAdults),
...
# count AF vs. AM
temp <- groupComps %>%
  select(species, AF, AM) %>%
  mutate(test = str_c(AF, "_", AM)) %>%
...
groupComps %>%
  ...
  group_by(species) %>%
  summarise(
    sumEqual = sum(af.am == "equal"),
    sumAFGreater = sum(af.am == "afGreater"),
    sumAMGreater = sum(af.am == "amGreater") 
  ) %>%
  as.data.frame() %>%
  mutate(
    prop_equal = sumEqual / (sumEqual + sumAFGreater + sumAMGreater),
    prop_afGreater = sumAFGreater / (sumEqual + sumAFGreater + sumAMGreater),
    prop_amGreater = sumAMGreater / (sumEqual + sumAFGreater + sumAMGreater)
    )
```

Are # of AFs signif diff from AMs?

```{r}
# LWED
statData_af.am_lwed <- groupComps %>%
  filter(totalIndivs > 1) %>%
  filter(species == "LWED") %>%
  select(groupName, AF, AM) %>%
  pivot_longer(-groupName,
               names_to = "sex",
               values_to = "count")

shapiro.test(statData_af.am_lwed$count) # p = 2.689e-10

# "less" asks: is number of males signif greater than # of females?
statData_af.am_lwed2 <- groupComps %>%
  filter(totalIndivs > 1) %>%
  filter(species == "LWED")

wilcox.test(statData_af.am_lwed2$AF, statData_af.am_lwed2$AM, paired = T, alternative = "less") # p = 4.298e-05
rstatix::pairwise_wilcox_test(statData_af.am_lwed, count ~ sex, pairwise = T)


# SIMP
statData_af.am_simp <- groupComps %>%
  filter(totalIndivs > 1) %>%
  filter(species == "SIMP") %>%
  select(groupName, AF, AM) %>%
  pivot_longer(-groupName,
               names_to = "sex",
               values_to = "count")
shapiro.test(statData_af.am_simp$count) # p = 5.443e-09

# "less" asks: is number of males signif greater than # of females?
statData_af.am_simp2 <- groupComps %>%
  filter(totalIndivs > 1) %>%
  filter(species == "SIMP")
wilcox.test(statData_af.am_simp2$AF, statData_af.am_simp2$AM, paired = T, alternative = "less") # p = 0.0908

rstatix::pairwise_wilcox_test(statData_af.am_simp, count ~ sex, pairwise = T)
```
...
```{r}
source("project_scripts/ggplot_theme_Publication-2.R")
#axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),

plot_groupComps_lwed <- groupComps %>%
  select(-c(totalAdults, totalIndivs, groupName)) %>%
  filter(species == "LWED") %>%
  pivot_longer(-c(species, groupName_anon, captureYear),
               names_to = "sex",
               values_to = "count") %>%
  ggplot(aes(x = count,
             y = captureYear,
             fill = sex,
             group = groupName_anon)) +
  geom_bar(stat = "identity") +
  #geom_line() +
  facet_wrap(vars(groupName_anon)) +
  #theme_bw()
  ...
  labs(x = "Number of individuals",
       y = "Year",
       fill = element_blank()) +
  scale_fill_manual(values = c("#4C413FFF",
                               "#278B9AFF",
                               "#E75B64FF",
                               "#D8AF39FF"), labels = c("AF" = "Adult female",
                            "AM" = "Adult male",
                            "JF" = "Juvenile female",
                            "JM" = "Juvenile male"))
plot_groupComps_simp <- groupComps %>%
  select(-c(totalAdults, totalIndivs, groupName)) %>%
  filter(species == "SIMP") %>%
  pivot_longer(-c(species, groupName_anon, captureYear),
               names_to = "sex",
...
plot_groupComps_lwed
plot_groupComps_simp
```

helpful to figure out what to set hjust/vjust: https://stackoverflow.com/questions/7263849/what-do-hjust-and-vjust-do-when-making-a-plot-using-ggplot

**AF vs. AM**

Number of adult females recorded in a group relative to number of adult males. 

```{r}
plotData_af.vs.am <- groupComps %>%
  select(species, groupName, AF, AM) %>%
  pivot_longer(-c(species, groupName),
               names_to = "sex",
               values_to = "count") %>%
  mutate(
    species = case_when(
      species == "LWED" ~ "Saddlebacks",
      .default = "Emperors"
      ),
    species = factor(species, levels = c("Saddlebacks", "Emperors")),
    sex = case_when(
      sex == "AF" ~ "Adult females",
      .default = "Adult males"
      )
    )

plot_af.vs.am <- plotData_af.vs.am %>%
  ggplot(aes(x = sex, y = count)) +
  geom_violin(width = 1, aes(fill = sex)) +
  scale_fill_manual(values = c("#4C413FFF", "#278B9AFF")) +
  geom_boxplot(width = 0.1, color = "black", alpha = 0.5, outliers = F) +
  geom_jitter(size=1, alpha=0.5, width = 0.3) +
...
## 4.3 New parity & inferred maternity

Based on groupComps -- if only one female in the group when juvs were there + in the year prior, female = parous AND likely mother of those juvs

Looks like all of the females who meet these criteria were already designated as parous by nipple measures anyway though - but at least we have more maternity assignments? -ish?

```{r}
parity_groupComps <- groupComps %>%
  
  mutate(yearPrev = as.numeric(captureYear) - 1,
         yearPrev = as.character(yearPrev)) %>%
  merge(., groupComps[, c("captureYear", "groupName", "AF")], by.x = c("yearPrev", "groupName"), by.y = c("captureYear", "groupName"), all.x = T, suffixes = c("", "prev")) %>%
  
  mutate(totalJuvs = JF + JM) %>%
  filter(totalJuvs > 0) %>%
  filter(AF == 1) %>%
  filter(AFprev == 1) %>%
  merge(., capData_2009to2019[, c("animalID", "sex", "captureYear", "groupName", "ageClass")], by.x = c("captureYear", "groupName"), by.y = c("captureYear", "groupName"), all.x = T) %>%
  filter(sex == "F") %>%
  filter(ageClass != "juvenile") %>%
  merge(., capData_2009to2019[, c("animalID", "sex", "captureYear", "groupName", "ageClass")], by.x = c("yearPrev", "groupName"), by.y = c("captureYear", "groupName"), all.x = T, suffixes = c("", "_prev")) %>%
  filter(sex_prev == "F") %>%
  filter(ageClass_prev != "juvenile") %>%
  select(-sex_prev) %>%
  mutate(
    prevCurrent_match = animalID == animalID_prev
  ) %>%
  filter(prevCurrent_match == T) %>%
  select(-prevCurrent_match) %>%
  merge(parityAssigments_firstParous[, c("animalID", "parityYear")], by = "animalID", all.x = T) %>%
  distinct() %>%
  mutate(parityCheck = as.numeric(parityYear) <= as.numeric(captureYear)) %>%
  relocate(species, animalID, animalID_prev, captureYear, yearPrev, groupName) %>%
  relocate(AFprev, .after = AF) %>%
  relocate(totalJuvs, .after = totalAdults)
```

**Inferred maternity:**

from Mini's dss -- 76F is mom of 2009 juvs; 117 allonursed them tho

```{r}
maternity_groupComps <- capData_2009to2019 %>%
  filter(ageClass == "juvenile") %>%
  filter(str_c(captureYear, groupName) %in% str_c(parity_groupComps$captureYear, parity_groupComps$groupName)) %>%
  select(species, animalID, sex, groupName, captureYear) %>%
  # assign inferredMat
  merge(., parity_groupComps[, c("animalID", "sex", "groupName", "captureYear")], by = c("captureYear", "groupName"), all.x = T, suffixes = c("_off", "_mat")) %>%
  relocate(species, groupName, captureYear, animalID_mat, animalID_off) %>%
  #dplyr::rename("sex_off" = "sex") %>%
  arrange(species, groupName, captureYear)
```

## 4.4 Known dispersers

Perhaps of note - two records of indivs who dispersed twice (both SIMP); SIMP_4M & SIMP_15F

interesting that 15F went first from DangerousPeople to Loners then stayed in Bees the rest of the time (2013-2019)

```{r}
# list of known dispersers (indivs captured in at least 2 diff groups)
knownDispersers <- presenceMatrix_2009to2019 %>%
  mutate(
    uniqueGroups = apply(select(., `2009`:`2019`),
                         1,
                         function(x) length(unique(x[!is.na(x)]))
                         )
    ) %>%
  filter(uniqueGroups >= 2)
...
```{r}
# 17 lwed groups, 9 simp groups
capData_2009to2023 %>%
  select(species, groupName) %>%
  # ditch "UNK" grp for now
  filter(!groupName %in% c("Loners", "UNK")) %>%
  distinct() %>%
  group_by(species) %>%
  summarise(nGroups = n())
...
lwed_grpSizes_2009to2023 <- capData_2009to2023 %>%
  filter(species == "LWED") %>%
  group_by(groupName, captureYear) %>%
  summarise(count = n()) %>%
  filter(groupName != "Loners")
round(mean(lwed_grpSizes_2009to2023$count)) # 5

simp_grpSizes_2009to2023 <- capData_2009to2023 %>%
  filter(species == "SIMP") %>%
  group_by(groupName, captureYear) %>%
  summarise(count = n()) %>%
  filter(groupName != "Loners")
round(mean(simp_grpSizes_2009to2023$count)) # 5
```
...
pidThreshLines_forFig <- data.frame(species = c("Saddlebacks", "Emperors"),
                                    vline = c(lwed_pid.thresh,
                                              simp_pid.thresh)) %>%
  mutate(species = factor(species, levels = c("Saddlebacks", "Emperors")))
pidSibThreshLines_forFig <- data.frame(species = c("Saddlebacks", "Emperors"),
                                    vline = c(lwed_pidSibs.thresh,
                                              simp_pidSibs.thresh)) %>%
  mutate(species = factor(species, levels = c("Saddlebacks", "Emperors")))

# PLOT
plot_pidPerm <- pidPerm_data_forFig %>%
  ggplot(aes(y = value,
             x = loci,
             color = statistic)) +
  geom_boxplot() +
  #scale_colour_Publication() +
  scale_color_manual(values = c("#278B9AFF", "#E75B64FF")) +
  theme_Publication() +
...
lwed_genoList_genind_adultsOnly <- get_genos_perYear(
...
lwed_genoList_genind_allIndivs <- get_genos_perYear(
  genoData_df = lwed_genos,
  genos_startCol = 7,
...
# now get the geno data for each subset
## get geno data for adults present in each year; format as genind
simp_genoList_genind_adultsOnly <- get_genos_perYear(
...
simp_genoList_genind_allIndivs <- get_genos_perYear(
  genoData_df = simp_genos,
  genos_startCol = 7,
...
Using hierfstat::basic.stats

Ho - observed heterozygosity; proportion of heterozygotes observed at each locus
Hs - gene diversity w/in subpops (expected heterozygosity w/in groups); calculated from the squared allele freqs
Ht - gene diversity across entire pop; total expected heterozygosity
Dst - genetic diversity b/t subpops
Htp - total gene diversity accounting for diffs among pops
...
Fstp - similar to Fst, but accounts for differentiation b/t pops
...
**NOTE**
Hexp from adegenet::summary represents the expected heterozygosity under HWE + accounts for all alleles
Hs from hierfstat::basic.stats = Nei's gene diversity, which is w/in pop heterozygosity averaged across loci weighted by pop size (accounts for subpop structure)

**Function:**
```{r}
get_multi.basicStats <- function(genoList_genind, diploid = T, digits = 4, fis.byGroup = T, nboot = 100) {
  
  # get necessary packages
  library(hierfstat)
  
  # if want overall stats only
  if (fis.byGroup == F) {
    
    # initialize df for results by year
    overall.stats_results_perYear <- data.frame()
    ...
    for (year_name in names(genoList_genind)) {
      ...
      year_overall.stats <- hierfstat::basic.stats(year_data, digits = digits)
      
      year_overall.stats_results <- year_overall.stats$overall %>%
        as.data.frame() %>%
        `colnames<-`(year_name) %>%
        t() %>%
        as.data.frame() %>%
      ...
      overall.stats_results_perYear <-
        dplyr::bind_rows(overall.stats_results_perYear,
                         year_overall.stats_results)
    
    }
    
    return(overall.stats_results_perYear)
    
  }
  
  # if also want fis stats per group
  if (fis.byGroup == T) {
    
    # initialize df for results by year
    overall.stats_results_perYear <- data.frame()
    fis.byGroup_results_perYear <- data.frame()
    ...
    # get fis by group & bootstrap for CIs
    year_fis.byGroup <- year_overall.stats$Fis
    year_fis.boot.byGroup <- boot.ppfis(year_data, nboot = nboot, digits = digits)
    
    # assemble byGroup dfs
    year_fis.boot.byGroup_df <- year_fis.boot.byGroup$fis.ci %>%
      `colnames<-`(c("fis.lcl", "fis.ucl")) %>%
      mutate(groupName = levels(year_data$pop)) %>%
      relocate(groupName)
    
    year_fis.byGroup_results <- year_fis.byGroup %>%
      as.data.frame() %>%
      ...
      pivot_longer(-locus,
                   ...
                   values_to = "fis") %>%
      group_by(groupName) %>%
      summarise(fis.mean = mean(fis, na.rm = T)) %>%
      merge(., year_fis.boot.byGroup_df, by = "groupName") %>%
      mutate(year = year_name) %>%
      relocate(year)
      
      fis.byGroup_results_perYear <- dplyr::bind_rows(fis.byGroup_results_perYear, year_fis.byGroup_results)
      
    }
    
    return(list(overall.stats_results_perYear, fis.byGroup_results_perYear))
...
**Run function:**
```{r}
lwed_multi.basicStats <- get_multi.basicStats(lwed_genoList_genind_adultsOnly, digits = 2, fis.byGroup = T, nboot = 1000)

simp_multi.basicStats <- get_multi.basicStats(simp_genoList_genind_adultsOnly, digits = 2, fis.byGroup = T, nboot = 1000)

lwed_multi.basicStats
simp_multi.basicStats

test <- basic.stats(lwed_genoList_genind_adultsOnly$`2015`)
```

### Table

```{r}
lwed_multi.basicStats[[2]] %>%
  ggplot(aes(x = year, y = fis.mean, color = groupName, group = groupName)) +
  geom_point() +
  geom_line()

lwed_multi.basicStats[[1]] %>%
  ggplot(aes(x = year, y = Fis)) +
  geom_point()
```
...
Levene's test is an alternative to the Bartlett test to test whether the variance among two or more groups is equal (i.e., testing homogeneity of variance aka homoscedasticity b/t groups). Relative to the Bartlett test, Levene's test is less sensitive to departures from normality.
...
NOTE -- significant result may not actually indicate HWE departure; what this test is really saying is just that the variation in obs vs. exp heterozygosity values is significantly different; could be due to HWE departure but could also be due to other things (e.g., result of diffs in sampling strategy, pop structure, etc.)
  ...
  for (year_name in names(genoList_genind)) {
    ...
    year_result <- with(year_summary_df, leveneTest(value, type)) %>%
      ...
      mutate(year = year_name,
             ...
             NA.perc = year_summary$NA.perc,
             mean_Hobs = mean(year_summary$Hobs, na.rm = T),
             mean_Hexp = mean(year_summary$Hexp, na.rm = T)) %>%
      relocate(c(year, nInd, nGrps, NA.perc, mean_Hobs, mean_Hexp))
...
base::rbind(lwed_multi.leveneTest, simp_multi.leveneTest) %>%
  select(species, year, nInd, nGrps, mean_Hobs, mean_Hexp) %>%
  mutate(higherHobs = mean_Hobs > mean_Hexp)

base::rbind(lwed_multi.leveneTest, simp_multi.leveneTest) %>%
  select(species, year, nInd, nGrps, NA.perc, mean_Hobs, mean_Hexp) %>%
  group_by(species) %>%
  summarise(min.Hobs = min(mean_Hobs),
            max.Hobs = max(mean_Hobs),
            min.Hexp = min(mean_Hexp),
            max.Hexp = max(mean_Hexp),
            mean.NA.perc = mean(NA.perc))
```
...
table_leveneTest <- base::rbind(lwed_multi.leveneTest, simp_multi.leveneTest) %>%
  mutate(
    NA.perc = round(NA.perc, 2),
    mean_Hobs = round(mean_Hobs, 2),
    mean_Hexp = round(mean_Hexp, 2),
    F.value = round(F.value, 2),
    p.value = format.pval(p.value, digits = 2, eps = 0.001, nsmall = 3)) %>%
  select(-df) %>%
  mutate(species = case_when(
    species == "Saddleback tamarins" ~ "x",
    .default = "y"
  )) %>%
  pivot_wider(
    names_from = species,
    values_from = c(nInd, nGrps, NA.perc, mean_Hobs, mean_Hexp, F.value, p.value),
    names_sep = ".",
    names_vary = "slowest"
  ) %>%
  gt() %>%
  sub_missing(columns = everything(),
              rows = everything(),
              missing_text = "---") %>%
  tab_spanner(label = "Saddleback tamarins",
              columns = contains(".x")) %>%
  tab_spanner(label = "Emperor tamarins",
              columns = contains(".y")) %>%
  ...
  cols_label(
    ...
    nGrps.x = md("#Grps"),
    NA.perc.x = md("%NA"),
    mean_Hobs.x = md("mean.Hobs"),
    mean_Hexp.x = md("mean.Hexp"),
    F.value.x = md("_F_"),
    p.value.x = md("_p_"),
    ...
    nGrps.y = md("#Grps"),
    NA.perc.y = md("%NA"),
    mean_Hobs.y = md("mean.Hobs"),
    mean_Hexp.y = md("mean.Hexp"),
    F.value.y = md("_F_"),
    p.value.y = md("_p_")
  ) %>%
...
```{r}
hw.test(lwed_genoList_genind_adultsOnly$`2015`)
...
Heatmap showing loci that deviate from HWE. Grey boxes = locus was monomorphic that year, white box = indivs that year weren't genotyped at that locus

```{r}
# create "locus_anon" just based on numbers bc can't use "scales = "free"" with coord_fixed (to make squares)
temp1 <- lwed_multi.hweTest %>%
  select(locus) %>%
  distinct() %>%
  filter(locus %in% simp_multi.hweTest$locus) %>%
  mutate(locus_anon = row_number())
temp2 <- lwed_multi.hweTest %>%
  select(locus) %>%
  distinct() %>%
  filter(!locus %in% temp1$locus) %>%
  mutate(locus_anon = row_number() + 38)
temp3 <- simp_multi.hweTest %>%
  select(locus) %>%
  distinct() %>%
  filter(!locus %in% temp1$locus) %>%
  mutate(locus_anon = row_number() + 38)
...
plotData_hweHeatmap <- lwed_multi.hweTest %>%
  mutate(species = "LWED") %>%
  rbind(., simp_multi.hweTest %>% mutate(species = "SIMP")) %>%
  mutate(
    p.value = case_when(
      df == 0 ~ NA,
      .default = p.value
  ...
  select(species, locus, year, p.exact) %>%
  merge(., temp4, by = "locus", all.x = T)
  

plot_hweHeatmap <- plotData_hweHeatmap %>%
  mutate(
    species = case_when(
      species == "LWED" ~ "Saddlebacks",
      .default = "Emperors"
  ...
  ggplot(aes(x = factor(locus_anon), y = year, fill = p.exact, group = species)) +
  geom_tile() +
  #geom_text(aes(label = p.exact), color = "black") +
  coord_equal() +
  facet_grid(rows = vars(species)) +
  theme_Publication() +
  #paletteer::scale_fill_paletteer_c("grDevices::Fall", direction = 1) +
  ...
  theme(strip.background = element_rect(fill = "white", color = "black", linetype = "solid"))
plot_hweHeatmap # saved as "plot_hweHeatmap"
```

## 7.4 F-stats

Helpful info from Population Genetics Glossary thing (https://www.uwyo.edu/dbmcd/popecol/maylects/popgengloss.html):
...
get_multi.groupRel(lwed_genos_forRelated_adultsOnly, iterations = 1000, outdir = "./DISSERTATION/ch2_demography/02_results/lwed/")

get_multi.groupRel(simp_genos_forRelated_adultsOnly, iterations = 1000, outdir = "./DISSERTATION/ch2_demography/02_results/simp/")
```

## Assess results

```{r}
lwed_grpRel <- read.csv("./DISSERTATION/ch2_demography/02_results/lwed/related_summary.csv")
simp_grpRel <- read.csv("./DISSERTATION/ch2_demography/02_results/simp/related_summary.csv")

mean(lwed_grpRel$RelValue) # 0.0707462
median(lwed_grpRel$RelValue) # 0.1161
lwed_grpRel %>%
  #group_by(Year) %>%
  summarise(mean = mean(RelValue),
            min = min(RelValue),
            max = max(RelValue))
lwed_grpRel %>%
  group_by(Group) %>%
...
simp_grpRel %>%
  ...
  summarise(mean = mean(RelValue),
            sd = sd(RelValue),
            median = median(RelValue))

lwed_grpRel %>%
  ggplot(aes(x = factor(Year), y = RelValue, group = factor(Group))) +
...
# only two years where overall ptest > 0.05
lwed_grpRel %>%
  group_by(Year, OverallPTest) %>%
  summarise(nGrps = n()) %>%
  filter(OverallPTest > 0.05)

# only one year where overall ptest > 0.05
simp_grpRel %>%
  group_by(Year, OverallPTest) %>%
  summarise(nGrps = n()) %>%
  filter(OverallPTest > 0.05)
...
simp_genos_forHf <- get_genos_perYear(genoData_df = simp_genos,
                                      ...
                                      sampleRef = sampleRef,
                                      outputFormat = "hierfstat")
simp_genos_forHf$`2011`$genoData
```
...
multi_sexbias.test <- function(genoData_list, nperm = NULL, alternative = c("two.sided", "less", "greater")) {
  ...
  for (y in names(genoData_list)) {
    ...
    # grab genoData & sexData
    genoData <- genoData_list[[as.character(y)]]$genoData %>%
      select(-sampleID) %>%
      merge(., groupRef_2009to2019[, c("groupName", "groupNum")], by.x = "pop", by.y = "groupName") %>%
      mutate(pop = groupNum) %>%
      select(-groupNum)
    
    sexData <- genoData_list[[as.character(y)]]$sexData
    
    # run each test; use "try" to account for any errors thrown
    maic <- try(sexbias.test(genoData,
                             sexData,
                             nperm = nperm,
                             test = "mAIc",
    ...
    vaic <- try(sexbias.test(genoData,
                             sexData,
                             nperm = nperm,
                             test = "vAIc",
                             alternative = "two.sided"),
...
```{r}
temp <- lwed_genos_forHf$`2015`$genoData %>%
  select(-sampleID) %>%
  merge(., groupRef_2009to2019[, c("groupName", "groupNum")], by.x = "pop", by.y = "groupName") %>%
  mutate(pop = groupNum) %>%
  select(-groupNum)
...
sexbias.test(temp, lwed_genos_forHf$`2015`$sexData, test = "FIS", nperm = 10)
...
# export so I don't have to rerun this ever again
write.csv(lwed_sexbias.testResults, "./DISSERTATION/ch2_demography/02_results/lwed/lwed_sexbias.testResults.csv", row.names = F)
write.csv(simp_sexbias.testResults, "./DISSERTATION/ch2_demography/02_results/simp/simp_sexbias.testResults.csv", row.names = F)
...
```{r}
# format data
lwed_sexbias.testResults_wide <- lwed_sexbias.testResults %>%
  mutate(statistic = round(statistic, 3)) %>%
  select(year, test, statistic, p.value) %>%
  ...
  mutate(
    ...
    p = case_when(
      as.character(p) == "10" ~ "ns",
      .default = as.character(p)
      )
    ) %>%
  pivot_wider(id_cols = test,
              names_from = year,
              values_from = c(stat, p),
              names_sep = "_",
              names_vary = "slowest" # return value1_name1, value2_name1, value1_name2, value2_name2⁠
              ) %>%
  mutate(species = "Saddlebacks") %>%
  relocate(species)
  
simp_sexbias.testResults_wide <- simp_sexbias.testResults %>%
  mutate(statistic = round(statistic, 3)) %>%
  select(year, test, statistic, p.value) %>%
  dplyr::rename("stat" = "statistic",
                "p" = "p.value") %>%
  ...
  mutate(species = "Emperors") %>%
  relocate(species)

# TABLE
table_genoDispersal <- rbind(lwed_sexbias.testResults_wide, simp_sexbias.testResults_wide) %>%
...
#  tab_style(
#    style = list(cell_fill(color = "#D9AF6BFF")),
#    locations = list(
#      cells_body(columns = contains("stat"),
#                 rows = test %in% c("mAIc", "FIS") & as.numeric(stat_2011) < 0),
#      cells_body(columns = contains("stat"),
#                 rows = test %in% c("mAIc", "FIS") & as.numeric(stat_2012) < 0)
#      )
#  )
...
What's a reasonable amount of time to count indiv as "gone"? --use capHist to see largest amount of time b/t captures of a given indiv >> most amount of time b/t seeing an indiv is 3 years (e.g., 10001), so calc deathYear_est as captureYear_last + 4

```{r}
max_zero_gap <- capHist_2009to2019 %>%
  mutate(
    temp = str_match(ch, "10\\s*(.*?)\\s*1")[,1],
    zeroCount = str_count(temp, "0")
    )

max(max_zero_gap$zeroCount, na.rm = T) # max = 3

# create "disappearYear" for deathYear est.
disappearYear <- presenceMatrix_2009to2023 %>%
  mutate(deathYear_est = case_when(
    captureYear_last != 2023 ~ captureYear_last + 4,
    .default = NA
  )) %>%
  select(species, animalID, deathYear_est) %>%
  # add actual known deathYears & replace as needed
  merge(., deathAssignments, by = "animalID", all.x = T) %>%
  mutate(deathYear_est.x = case_when(
    !is.na(deathYear_est.y) ~ deathYear_est.y,
    .default = deathYear_est.x
  )) %>%
  select(-deathYear_est.y) %>%
  dplyr::rename("deathYear_est" = "deathYear_est.x")
```
...
lwed_genos_franz <- names(lwed_genos_franz_temp) %>%
  ...
  # add deathYear (including those estimated above)
  merge(., disappearYear[, c("animalID", "deathYear_est")], by = "animalID", all.x = T) %>%
...
number of candidate female/male parents in the population). 

As such, I'm using the sex-specific abundance estimates calculated using CMR "Model 3" (below).
...
FRANz --femrepro 2:30 --malerepro 2:30 --Nf 86 --Nm 97 --fullsibtest --fullsibparental --updatefreqs --poutformat 2 ./../../../00_data/lwed_genos_franz.dat
...
Filter by groupMatch == T + diff_birth.parity NA or at least not negative

LWED: F = 4, M = 4 SIMP: F = 1, M = 1

```{r}
lwed_po_run1_forPed <- lwed_po_run1 %>%
  filter(groupMatch == TRUE) %>%
  filter(is.na(diff_birth.parity) | diff_birth.parity >= 0) %>%
...
FRANz --femrepro 2:30 --malerepro 2:30 --Nf 86 --Nm 97 --pedigreein ./../../../00_data/lwed_poPed_forRun2.dat --fullsibtest --fullsibparental --updatefreqs --poutformat 2 ./../../../00_data/lwed_genos_franz.dat
...
lwed_po_run2 <- lwed_po_run2_file %>%
  ...
  select(rowID, poType, offspring, LOD, posterior, parent1, parent2) %>%
  
  pivot_longer(-c(rowID, poType, offspring, LOD, posterior),
               names_to = "parentID",
               values_to = "parent") %>%
  select(-parentID) %>%
  na.omit() %>%
  ...
  # add groups (set parent group = group in offspring birthYear - 1)
  mutate(
    animalID_par = gsub(".*_([0-9]+)$", "\\1", parent),
    animalID_off = gsub(".*_([0-9]+)$", "\\1", offspring)
    ) %>%
  ...
  relocate(rowID, parent, offspring, LOD, posterior, poType, animalID_par, animalID_off, sex_par, sex_off, birthYear_par, birthYear_off, diff_birthOff.birthPar, parityYear, diff_birth.parity, groupName_par, groupName_off, groupMatch) %>%
...
Checking mat assignments against inferred mat based on groupComps in 4.3 above -- the 6 offspring w/both franz assigned maternity + groupComp inferred maternity MATCHED for lwed

but 0/2 matched for simp...

```{r}
# LWED
lwed_po_run2_infMatChecks <- lwed_po_run2 %>%
  merge(., maternity_groupComps[, c("animalID_mat", "animalID_off", "sex_mat")], by.x = c("animalID_off", "sex_par"), by.y = c("animalID_off", "sex_mat"), all.x = T) %>%
  relocate(animalID_off, .after = animalID_par) %>%
  relocate(sex_par, .before = sex_off) %>%
  relocate(animalID_mat, .after = animalID_par) %>%
  filter(!is.na(animalID_mat)) %>%
  # filter to highest posterior when have >1 assignment per off
  group_by(animalID_off) %>%
  slice_max(posterior) %>%
  ungroup() %>%
  mutate(
    franzComp_match = animalID_par == animalID_mat
  ) %>%
  relocate(franzComp_match, .after = animalID_mat)

length(unique(lwed_po_run2_infMatChecks$animalID_off)) # 6/6 offspring confirmed

# SIMP
simp_po_run2_infMatChecks <- simp_po_run2 %>%
  merge(., maternity_groupComps[, c("animalID_mat", "animalID_off", "sex_mat")], by.x = c("animalID_off", "sex_par"), by.y = c("animalID_off", "sex_mat"), all.x = T) %>%
  relocate(animalID_off, .after = animalID_par) %>%
  relocate(sex_par, .before = sex_off) %>%
  relocate(animalID_mat, .after = animalID_par) %>%
...
length(unique(simp_po_run2_infMatChecks$animalID_off)) # 0/2 offspring confirmed
```

offspring 207 - genMat = 149; infMat = 15 - bees
offspring 211 - genMat = 34; infMat = 54 - whiteemps
...
scratch work--
```{r}
temp <- lwed_po_run2_lod1_juvsOnly %>%
  mutate(parType = case_when(
    sex_par == "F" ~ "dam",
    .default = "sire"
  )) %>%
  select(animalID_off, parType, animalID_par) %>%
  pivot_wider(id_cols = animalID_off,
              names_from = parType,
              values_from = animalID_par) %>%
  mutate(
    poType = case_when(
      !is.na(dam) & !is.na(sire) ~ "trio",
      !is.na(dam) & is.na(sire) ~ "sire",
      is.na(dam) & !is.na(sire) ~ "dam"
    )
  )
```
...
# ...only two sibling sets per species that had parentage assignments for all indivs
# at least it was the maybe triplet lwed group? 161, 162, 163
# so in total:
## LWED: 60/62 & 161/162/163
## SIMP: 288/289 & 167/168
...
LWED 60/62
- 60M - mom = 63F, no dad assigned
- 62M - mom = 97F, dad = 58F

SIMP 167/168
- 167M - mom = 33F
- 168F - mom = 34F (unlikely bc 34F not parous til 2 yrs later; also 34F seems to be daughter of 33F)
...
lwed_po.twinList_60.62 <- lwed_po_run2 %>%
  filter(animalID_off %in% c(60, 62))

simp_po.twinList_checks <- simp_po_run2 %>%
  filter(animalID_off %in% c(167, 168, 288, 289))
...
pdf(file = "./DISSERTATION/ch2_demography/02_results/lwed/ped_full_lwed.pdf", width = 20, height = 20)
plot(lwed_trios_ped) # too big
dev.off()


writePed(lwed_trios_ped$`_comp1`, prefix = "./DISSERTATION/ch2_demography/02_results/lwed/pedTrios_comp1", what = "ped", famid = F)
...
temp <- lwed_po_run2_lod2_juvsOnly %>%
  filter(sex_par == "F")
...
tempX <- groupMembership %>%
 ...
 #             values_from = "groupName") %>%
  ...
  distinct() %>%
  arrange(species, animalID, captureYear)


temp <- tempX %>%
  filter(groupName == "Sun7") %>%
  group_by(animalID) %>%
  slice(c(1, n())) %>%
  ungroup() %>%
  mutate(id = rep(c("captureYear_first","captureYear_last"), 14)) %>%
  pivot_wider(id_cols = c(animalID, sex, birthYear_est),
              names_from = id,
              values_from = captureYear) %>%
  mutate(captureYear_first = as.numeric(captureYear_first),
         captureYear_last = as.numeric(captureYear_last))
class(temp$birthYear_est)
class(temp$captureYear_first)

temp %>%
  ggplot() +
  geom_point(aes(x = birthYear_est,
                 y = animalID,
                 color = sex)) +
  geom_segment(aes(x = captureYear_first,
                   xend = captureYear_last,
                   y = animalID,
                   yend = animalID,
                   color = sex))
  

View(capData_2009to2019 %>% filter(animalID == 272))
...
```{r}
# all juvs recorded
temp1 <- rbind(lwed_po_run2_lod2_juvsOnly, simp_po_run2_lod2_juvsOnly)

temp2 <- sampleRef %>%
  select(species, animalID, birthYear_est, natalGroup) %>%
  na.omit() %>%
  distinct() %>%
  #filter(species == "LWED") %>%
  merge(., temp1[, c("animalID_off", "animalID_par", "sex_par", "groupName_par")], by.x = "animalID", by.y = "animalID_off", all.x = T) %>%
  filter(is.na(sex_par) | sex_par == "F") %>%
  merge(., presenceMatrix[, c("animalID", "captureYear_last")], by = "animalID") %>%
  arrange(species, natalGroup, birthYear_est)

presenceMatrix_2009to2019
groupRef_2009to2019

lwed_po_run2_lod2_juvsOnly

twinList
sampleRef

View(capData_2009to2023 %>% filter(animalID == 273))
View(capData_2009to2023 %>% filter(groupName == "AR6"))
```
...
capOverview_continuous_2009to2023 <- capData_2009to2023 %>%
  select(captureDate, captureYear, species, groupName, animalID) %>%
  distinct()

# only keep one per year
capOverview_discrete_2009to2023 <- capData_2009to2023 %>%
  ...
  # anonymize groupName
  merge(., groupRef_2009to2019[, c("groupName", "groupName_anon")], by = "groupName") %>%
  arrange(species, desc(groupName_anon)) %>%
  mutate(groupName_anon = factor(groupName_anon, ordered = T))

# plot
plot_capOverview_byGroup_2009to2023 <- capOverview_discrete_2009to2023 %>%
  mutate(species = case_when(
    species == "LWED" ~ "Saddleback tamarins",
    .default = "Emperor tamarins"
  ),
  species = factor(species, levels = c("Saddleback tamarins", "Emperor tamarins"))
  ) %>%
  ggplot() +
  geom_segment(aes(x = captureYear,
                   xend = nextCapture,
                   ...
                   yend = groupName_anon,
                   color = as.factor(capID))) +
  geom_point(aes(x = captureYear, y = groupName_anon, color = as.factor(capID), size = nCaptured)) +
...
plot_capOverview_byGroup_2009to2023
```
...
```{r}
# keep all cap records
capOverview_continuous_2009to2019 <- capData_2009to2019 %>%
  select(captureDate, captureYear, species, groupName, animalID) %>%
  distinct()

# only keep one per year
capOverview_discrete_2009to2019 <- capData_2009to2019 %>%
  select(captureYear, species, groupName, animalID) %>%
  distinct() %>%
  # add first year captured
  arrange(species, groupName, captureYear) %>%
...
# plot
plot_capOverview_byGroup_2009to2019 <- capOverview_discrete_2009to2019 %>%
  mutate(species = case_when(
    species == "LWED" ~ "L. w. weddelli",
    .default = "T. subgrisescens"
  ),
  species = factor(species, levels = c("L. w. weddelli", "T. subgrisescens"))
  ) %>%
  ggplot() +
  geom_segment(aes(x = captureYear,
                   xend = nextCapture,
...
plot_capOverview_byGroup_2009to2019
```
...
```{r}
data_table_m1_lwed.cjs.models <- m1.p_lwed.cjs.models$model.table %>%
  select(-Phi, -p, -AICc) %>%
  ...
  relocate(rank)
table_m1_lwed.cjs.models <- data_table_m1_lwed.cjs.models %>%
  gt() %>%
  cols_label(
    rank = md("**Rank**"),
    model = md("**Model**"),
...
```{r}
data_table_m1_simp.cjs.models <- m1.p_simp.cjs.models$model.table %>%
  select(-Phi, -p, -AICc) %>%
  mutate(rank = row_number(),
         DeltaAICc = round(DeltaAICc, 2),
         weight = round(weight, 2),
         Deviance = round(Deviance, 2)) %>%
  relocate(rank)
table_m1_simp.cjs.models <- data_table_m1_simp.cjs.models %>%
  gt() %>%
  cols_label(
    rank = md("**Rank**"),
    model = md("**Model**"),
...
COMBINE LWED/SIMP TABLES

```{r}
data_table2.5 <- data_table_m1_lwed.cjs.models %>%
  mutate(species = "Saddleback tamarins") %>%
  rbind(., data_table_m1_simp.cjs.models %>% mutate(species = "Emperor tamarins"))

table2.5 <- data_table2.5 %>%
  filter(DeltaAICc <= 3) %>%
  group_by(species) %>%
  gt() %>%
  cols_label(
...
table2.5 %>%
  gtsave("./DISSERTATION/ch2_demography/02_results/00_tablesFigures/table2.5.png")
```
...
```{r}
# list of known dispersers created above
knownDispersers
...
Difference in dispersal timing b/t twins born in same group?

```{r}
m2_capHist

sampleRef %>%
  select(species, sex, animalID, natalGroup, birthYear_est) %>%
  na.omit() %>%
  distinct()
...
groupComps %>%
  mutate(totalJuvs = JF + JM) %>%
  group_by(species) %>%
  summarise(median = median(totalJuvs),
            count1 = sum(totalJuvs == 1),
            count2 = sum(totalJuvs == 2),
            count0 = sum(totalJuvs == 0))
```
...
```{r}
plotData_fmPhilopatry <- rbind(
  femalephilopatry_forPlot %>% mutate(sex = "Female",
                                      species = "Saddlebacks"),
  malephilopatry_forPlot %>% mutate(sex = "Male",
                                    species = "Saddlebacks"),
  simpF.philopatry_forPlot %>% mutate(sex = "Female",
                                      species = "Emperors"),
  simpM.philopatry_forPlot %>% mutate(sex = "Male",
                                      species = "Emperors")) %>%
  mutate(species = factor(species, levels = c("Saddlebacks", "Emperors")))

plotData_fmPhilopatry %>%
  mutate(Time = as.numeric(Time)) %>%
  ggplot() +
  geom_line(aes(x = Time, y = expPrevalence, group = sex, color = sex), size = 1) +
  geom_ribbon(aes(x = Time, ymin = l95ci, ymax = u95ci, group = sex, fill = sex), alpha = 0.25) +
  #theme_Publication_rv() +
  theme_classic() +
  scale_color_manual(values = c("Female" = "#4C413FFF", "Male" = "#278B9AFF")) +
  scale_fill_manual(values = c("Female" = "#4C413FFF", "Male" = "#278B9AFF")) +
  labs(x = "Time since first captured (years)",
       y = "Probability of presence in natal group (%)",
       color = element_blank(),
       fill = element_blank()) +
...
lwedF_sojourn <- sojourn.msm(lwed_philopatry.msm, covariates = list(sex = "F"))

lwedM_sojourn <- sojourn.msm(lwed_philopatry.msm, covariates = list(sex = "M"))
...
birthData %>%
  ...
  distinct() %>%
  filter(species == "LWED") %>%
  select(age_watsaDiss_months, sex) %>%
  na.omit() %>%
  mutate(ageMonths = age_watsaDiss_months/12) %>%
  group_by(sex) %>%
  summarise(
    n = n(),
    meanAge_years = mean(ageMonths),
    sdAge_years = sd(ageMonths),
    minAge_months = min(age_watsaDiss_months),
    maxAge_months = max(age_watsaDiss_months)
  )

# numbers are from table above^^
(0.393 - 0.264) + (lwedF_sojourn$estimates - lwedF_sojourn$SE) # 1.255813 yrs
(0.393 + 0.264) + (lwedF_sojourn$estimates + lwedF_sojourn$SE) # 2.6328 yrs

(0.567 - 0.189) + (lwedM_sojourn$estimates - lwedM_sojourn$SE) # 1.975707 yrs
(0.567 + 0.189) + (lwedM_sojourn$estimates + lwedM_sojourn$SE) # 3.136409 yrs
```
...
```{r}
# mean sojourn times for males and females in their natal group
simpF_sojourn <- sojourn.msm(simp_philopatry.msm, covariates = list(sex = "F"))

simpM_sojourn <- sojourn.msm(simp_philopatry.msm, covariates = list(sex = "M"))
...
# numbers are from table above^^
(0.470 - 0.171) + (simpF_sojourn$estimates - simpF_sojourn$SE) # 3.091203 yrs
(0.470 + 0.171) + (simpF_sojourn$estimates + simpF_sojourn$SE) # 5.854625 yrs

(0.484 - 0.240) + (simpM_sojourn$estimates - simpM_sojourn$SE) # 1.535228 yrs
(0.484 + 0.240) + (simpM_sojourn$estimates + simpM_sojourn$SE) # 3.039199 yrs
```
...
# superpopulation for F and M
m3_lwed.js.derived$NGross # 85.96187 96.50888
# N by sex each year
m3_lwed.js.derived$N 
# total N each year
m3_lwed.js.derived$Nbyocc # year11 = 46.72 +/1 SE 3.75

m3_simp.js.derived$NGross # 57.02435 63.52291
m3_simp.js.derived$N
m3_simp.js.derived$Nbyocc # year9 = 44.52 +/0 SE 5.12

# pop density based on these numbers:
## lwed
46.72/5 # 9.344
44.52/5 # 8.904
```
...
# total cap per year:
temp <- rbind(totalIndivs_perYear_lwedF %>% mutate(species = "Saddlebacks",
                                                   sex = "F"),
              totalIndivs_perYear_lwedM %>% mutate(species = "Saddlebacks",
                                                   sex = "M"),
              totalIndivs_perYear_simpF %>% mutate(species = "Emperors",
                                                   sex = "F"),
              totalIndivs_perYear_simpM %>% mutate(species = "Emperors",
                                                   sex = "M")) %>%
  dplyr::rename("Occasion" = "year")

plotData_m3_popSize <- rbind(m3_lwed_forPlot, m3_simp_forPlot) %>%
  merge(., temp, by = c("species", "Occasion", "sex")) %>%
  mutate(species = factor(species, levels = c("Saddlebacks", "Emperors"))) %>%
  arrange(species, Occasion)

plot_m3_popSize <- plotData_m3_popSize %>%
  ...
  # add line for total captured - jk that's v ugly
  #geom_line(aes(x = Occasion, y = totalIndivs, color = sex)) +
  
  # Axis labels
  labs(x = "Year", y = "Population Estimate (N)", color = "Sex", fill = "Sex") +
  ...
  scale_color_manual(values = c("F" = "#4C413FFF", "M" = "#278B9AFF")) +
  scale_fill_manual(values = c("F" = "#4C413FFF", "M" = "#278B9AFF")) +
  facet_grid(cols = vars(species)) +
...
**Create initial input df**

I'm assigning females as "breeder" based on parity designations from nipple length measurements; while I did check to see if any additional females could be designated as parous based on group composition (i.e., for year when juvs first captured, a particular female was the only one present that year and the only one present in the year prior) - HOWEVER, the females I identified through that method were already designated as parous by nipple measures anyway (see section 4.3 New parity & inferred maternity)

**NOTE** - based on franz results, lwed_202F appears to be mother of 270F in 2019 in WhiteSf - this was the only franz result that I could confidently use for female parents born during the study

```{r}
# fullset minus ghost records
m1_capHist_temp <- capData_2009to2019[, c("animalID", "captureDate", "captureYear")] %>%
  # ensure only one cap per year; choose the first one
  group_by(animalID, captureYear) %>%
  slice_max(captureDate) %>%
  ungroup() %>%
  select(-captureYear) %>%
  dplyr::rename("captureDate_capYear" = "captureDate") %>%
  ...
  mutate(
    statecode = case_when(
      # males still in natal grp
      sex == "M" & groupName_capYear == groupName_birthYear ~ "natal",
      sex == "M" & groupName_capYear != groupName_birthYear ~ "dispersed",
      # 27_M left natal group, then came back in 2017; recode those yrs to knownDispersal
      animalID == 27 & captureYear %in% 2017:2019 ~ "dispersed",
      
      # females still in natal grp...
      ## & non-breeder
      sex == "F" & groupName_capYear == groupName_birthYear & is.na(parityYear) ~ "natal", 
      sex == "F" & groupName_capYear == groupName_birthYear & captureYear < parityYear ~ "natal", 
      ## & breeder
      sex == "F" & groupName_capYear == groupName_birthYear & captureYear >= parityYear ~ "natal.breeder",
      
      # females dispersed...
      ## & just existing
      sex == "F" & groupName_capYear != groupName_birthYear & is.na(parityYear) ~ "dispersed",
      sex == "F" & groupName_capYear != groupName_birthYear & captureYear < parityYear ~ "dispersed", 
      ## & breeder
      sex == "F" & groupName_capYear != groupName_birthYear & captureYear >= parityYear ~ "dispersed.breeder",
  ...
  # manually adjust animalID 202 as breeder based on franz results
  mutate(
    statecode = case_when(
      animalID == 202 & str_sub(captureDate, 1, 4) == 2019 ~ "dispersed.breeder",
      .default = statecode
    )
  )
```
...
Keep to 4 states (1 = confirmed natal group, 2 = dispersed, 3 = breeder, 4 = died/dispersed from pop) -- I've included statecodes though w/more precision if want to try other states at some point

```{r}
# create "ghost" capture records for indivs immediately following last known capture
m1_capHist_ghostRecords <- m1_capHist_temp %>%
  # extract last known captureDate
  group_by(animalID) %>%
  slice_max(captureDate) %>%
  ungroup() %>%
  mutate(captureYear = str_sub(captureDate, 1, 4)) %>%
  merge(., capPeriods_byYear[, c("captureYear", "capID")], by = "captureYear") %>%
  dplyr::rename("last_captureDate" = "captureDate",
                "last_captureYear" = "captureYear",
                "last_statecode" = "statecode",
                "last_capID" = "capID",
                "last_groupName" = "groupName") %>%
  select(animalID, species, sex, last_captureDate, last_captureYear, last_statecode, last_capID, last_groupName, captureDate_firstCap) %>%
  # adjust last_capIDs for knownDeaths
  merge(., disappearYear[, c("animalID", "deathYear_est")], by = "animalID", all.x = T) %>%
  merge(., capPeriods_byYear[, c("captureYear", "capID")], by.x = "deathYear_est", by.y = "captureYear", all.x = T) %>%
  # add capPeriods for ghost records (cap at 2019)
  mutate(
    ghost_capID = last_capID + 1,
    ghost_capID = case_when(
      # cap at 2019
      ghost_capID > 11 ~ 11,
      # recode known deaths to capID from deathYear (added above)
      animalID %in% deathAssignments$animalID ~ capID,
      .default = ghost_capID
    )
  ) %>%
  select(-capID) %>%
  merge(., capPeriods_byYear, by.x = "ghost_capID", by.y = "capID") %>%
  # add statecodes
  mutate(
    statecode = case_when(
      animalID %in% deathAssignments$animalID ~ "knownDeath",
      last_captureYear == 2019 ~ last_statecode,
      deathYear_est <= 2019 ~ str_c("gone4plus_", last_statecode),
      .default = str_c("gone3less_", last_statecode)
    )
  ) %>%
  # define new captureDate
  mutate(
    captureDate = case_when(
      # if indiv has deathYear_est, set to capPeriod_end for that year's cap season
      statecode == "knownDeath" ~ capPeriod_end,
      # if last capYear is 2019, set to ending date of 2019 cap season
      last_captureYear == 2019 ~ capPeriod_end,
      # if last capYear < 2019 & set to starting capture date of next cap season
      last_captureYear < 2019 ~ capPeriod_start,
      # if group last capYear < 2019 & not captured again til after 2019, set to starting date of 2019 cap season
      is.na(deathYear_est) ~ capPeriod_start,
      .default = NA
      ),
    # if captureDate_capYear (ghostYear) == captureDate (last year), add a day to captureDate_capYear
    captureDate = case_when(
      as.Date(captureDate) == as.Date(last_captureDate) ~ as.Date(captureDate) + 1,
      .default = as.Date(captureDate)
    ),
    # switch back to character data type
    captureDate = as.character(captureDate),
    last_captureDate = as.character(last_captureDate)
  ) %>%
  # organize a bit
  mutate(
    censor = NA,
    state = NA,
    Time.days = as.numeric(as.Date(captureDate) - as.Date(captureDate_firstCap)),
    Time.years = Time.days / 365
    ) %>%
  select(species, sex, animalID, last_groupName, captureDate, capPeriod_start, capPeriod_end, captureDate_firstCap, censor, Time.days, Time.years, statecode, state) %>%
  dplyr::rename("groupName" = "last_groupName") %>%
  arrange(as.numeric(animalID), captureDate) 
```

**FULL SET**

For statecodes, combine natal.breeder and dispersed.breeder into one since there aren't many instances of either (see general metrics)
Also, if gone > 3 years, assign state to 4 (same as knownDeath)

```{r}
# both females and males
m1_capHist_FM <- rbind(m1_capHist_temp, m1_capHist_ghostRecords) %>%
  arrange(as.numeric(animalID), captureDate) %>%
  # assign state
  mutate(
    state = case_when(
      statecode == "natal" ~ "1",
      statecode == "dispersed" ~ "2",
      
      statecode == "natal.breeder" ~ "3",
      statecode == "dispersed.breeder" ~ "3",
      
      statecode == "knownDeath" ~ "4",
      statecode == "gone4plus_natal" ~ "4",
      statecode == "gone4plus_dispersed" ~ "4",
      statecode == "gone4plus_natal.breeder" ~ "4",
      statecode == "gone4plus_dispersed.breeder" ~ "4",
      
      statecode == "gone3less_natal" ~ "99",
      statecode == "gone3less_dispersed" ~ "99",
      statecode == "gone3less_natal.breeder" ~ "4",
      statecode == "gone3less_dispersed.breeder" ~ "4",
...
# and subset to F only + make species subsets
m1_capHist <- m1_capHist_FM %>%
  filter(sex == "F")

m1_capHist_lwed <- m1_capHist %>%
  filter(species == "LWED")

m1_capHist_simp <- m1_capHist %>%
  filter(species == "SIMP")
```
...
```{r}
# 25 lwed females, 26 simp females
m1_capHist %>%
  group_by(species) %>%
  summarise(
    count = length(unique(animalID))
  )

# 2 instances in which female inherited breeding position, 2 LWED (animalIDs 7) & 1 SIMP (animalID 135)
m1_capHist %>%
  select(species, animalID, statecode) %>%
  filter(statecode == "natal.breeder") %>%
  distinct()

# 1 each where dispersed to breed; lwed animalID 202, SIMP animalID 34
m1_capHist %>%
  select(species, animalID, statecode) %>%
  filter(statecode == "dispersed.breeder") %>%
  distinct()
```
...
```{r}
fm.fates <- m1_capHist_FM %>%
  mutate(captureYear = str_sub(captureDate, 1, 4)) %>%
  group_by(animalID, captureYear) %>%
  slice_max(captureDate) %>%
  ungroup() %>%
  merge(., capPeriods_byIndiv[, c("animalID", "capPeriod_end")], by = "animalID", all.x = T, suffixes = c("", "_last")) %>%
  mutate(
    captureYear_firstCap = str_sub(captureDate_firstCap, 1, 4),
    captureYear_lastCap = as.numeric(str_sub(capPeriod_end_last, 1, 4)),
    captureYear_lastCap = case_when(
      animalID %in% deathAssignments$animalID ~ captureYear_lastCap,
      captureYear_lastCap < 2019 ~ captureYear_lastCap + 1,
      .default = captureYear_lastCap
    ),
    captureYear_lastCap = as.character(captureYear_lastCap),
    # Order animalID based on captureYear_firstCap
    animalID = fct_reorder(animalID, as.numeric(captureYear_firstCap))
  ) %>%
  arrange(captureYear_firstCap) %>%
  mutate(
    new_statecode = case_match(
      statecode,
      "gone3less_dispersed" ~ "censored",
      "gone3less_natal" ~ "censored",
      "gone4plus_natal" ~ "dead/gone",
      "gone4plus_dispersed" ~ "dead/gone",
      "gone4plus_natal.breeder" ~ "dead/gone",
      "knownDeath" ~ "dead/gone",
      .default = statecode
    ),
    new_statecode = factor(new_statecode, levels = c("natal", "dispersed", "censored", "natal.breeder", "dispersed.breeder", "dead/gone"))
  ) %>%
  # add natal group
  merge(., sampleRef[, c("animalID", "natalGroup")], by = "animalID", all.x = T) %>%
  distinct()

plot_fm.fates_lwed <- fm.fates %>%
  filter(species == "LWED") %>%
  mutate(species = "Saddlebacks",
         sex = case_match(
           sex,
  ...
  ggplot() +
  geom_segment(aes(x = captureYear_firstCap,
                   xend = captureYear_lastCap,
                   y = animalID,
                   yend = animalID)) +
  geom_point(aes(x = captureYear,
                 y = animalID,
                 color = new_statecode,
                 shape = new_statecode),
             size = 2.5,
  ...
  scale_color_manual(values = c("#5A6F80FF",
                                "#278B9AFF",
                                "#278B9AFF",
                                "#E75B64FF",
                                "#D8AF39FF",
                                "black"),
                     labels = c("Natal group",
                                "Dispersed",
                                "Censored",
                                "Natal breeder",
                                "Dispersed breeder",
...
m1_lwed_philopatry.df <- m1_capHist_lwed

table(m1_lwed_philopatry.df$statecode)

statetable.msm(state = state,
               subject = animalID,
               data = m1_lwed_philopatry.df)

summary(m1_lwed_philopatry.df$Time.years)

# set up the q matrix for the transitions (assume that individuals can transition from non-breeder to gone or known out of group)
m1_lwed_philopatry.q <- rbind(c(0, 0.5),
                              c(0, 0)) # assuming ~1 year in state 1 before disappearing
# ^^ gives same results for 1 & 0.5, suggesting that it's the true max-likelihood estimate
...
presenceMatrix old code:
```{r}
library(dplyr)
library(purrr)
conflicts_prefer(purrr::reduce)

presenceMatrix <- capData_2009to2023 %>%
  select(species, animalID, sex, captureYear) %>%
  arrange(animalID, captureYear) %>%
  group_by(animalID) %>%
  slice(c(1, n())) %>%
  ...
  pivot_wider(id_cols = c(animalID, species, sex),
              names_from = id,
              values_from = captureYear) %>%
  # list animalID as present for all years >= capYear_first & <= capYear_last
...
# get total indivs present in pop per year (based on presenceMatrix)
totalIndivs_perYear_lwed <- presenceMatrix %>%
  filter(species == "LWED") %>%
  select(contains("20")) %>%
  t() %>%
  as.data.frame() %>%
...
totalIndivs_perYear_lwedF <- presenceMatrix %>%
  filter(species == "LWED") %>%
  filter(sex == "F") %>%
  select(contains("20")) %>%
  t() %>%
  ...
  rownames_to_column("year") %>%
  select(year, totalIndivs)
totalIndivs_perYear_lwedM <- presenceMatrix %>%
  filter(species == "LWED") %>%
  filter(sex == "M") %>%
  select(contains("20")) %>%
  t() %>%
...
temp <- groupMembership_2009to2023 %>%
  #filter(species == "LWED") %>%
  select(contains("20")) %>%
  t() %>%
  as.data.frame() %>%
...
totalIndivs_perYear_simp <- presenceMatrix %>%
  filter(species == "SIMP") %>%
  select(contains("20")) %>%
  t() %>%
  as.data.frame() %>%
  ...
  rownames_to_column("year") %>%
  select(year, totalIndivs)
totalIndivs_perYear_simpF <- presenceMatrix %>%
  filter(species == "SIMP") %>%
  filter(sex == "F") %>%
  select(contains("20")) %>%
  t() %>%
  ...
  rownames_to_column("year") %>%
  select(year, totalIndivs)
totalIndivs_perYear_simpM <- presenceMatrix %>%
  filter(species == "SIMP") %>%
  filter(sex == "M") %>%
  select(contains("20")) %>%
  t() %>%
...
Perhaps of note -- 2 records of indivs who dispersed twice; both SIMP
...
capPeriods_byIndiv_lastCaps <- capData_2009to2019 %>%
  select(animalID, captureYear) %>%
  arrange(animalID, as.numeric(captureYear)) %>%
  group_by(animalID) %>%
  slice_tail(n = 2) %>%
  ...
  pivot_wider(id_cols = animalID,
              names_from = id,
              values_from = captureYear) %>%
  dplyr::rename("capYear_penult" = "1",
                "capYear_last" = "2") %>%
  mutate(capYear_last = case_when(
    is.na(capYear_last) ~ capYear_penult,
    .default = capYear_last
  ))
```
