...
capData_2009to2019_firstEntry <- capData[match(unique(capData$animalID), capData$animalID),] %>%
  filter(rowID <= 613) %>%
  select(rowID, captureDate, animalID, ageClass, animalName1, animalName2, groupName, species, sex, notes_MD, notes_RV) %>%
  # remove UNK animalID
  filter(animalID != "UNK")
...
**LWED** - nGroups = 16 ageClass F M Total juvenile 26 44 70 nonJuv 42
33 75 Total 68 77 145

**SIMP** - nGroups = 10 ageClass F M Total juvenile 26 27 53 nonJuv 22
27 49 Total 48 54 102
...
m1_lwed_capHist <- capHist_2009to2019 %>%
  ...
  # filter to those w/known birth years
  merge(., birthAssignments[, c("animalID", "birthYear_est")], by = "animalID") %>%
  # recode any captures in year preceding normalized birthYear to 0
  mutate(
    temp = birthYear_est - 2009
  ) %>%
  mutate(
    ch = `substr<-`(ch, temp, temp, "0")
  ) %>%
  # subset to 2010-2019 (since normalized birthYears start in 2010)
  mutate(
    ch = str_sub(ch, 2, -1)
  ) %>%
  select(ch, sex) %>%
...
m1_lwed_capIntervals <- lwed_capSummary_byGroup %>%
...
m1_lwed_capHist.r2Proc <- marked::process.data(m1_lwed_capHist,
                                               model = "CJS",
                                               begin.time = 2010,
                                               time.intervals = m1_lwed_capIntervals,
                                               accumulate = FALSE)

m1_lwed.hist <- matrix(as.numeric(unlist(strsplit(as.character(m1_lwed_capHist.r2Proc$data$ch), ","))),
                   nrow = length(m1_lwed_capHist.r2Proc$data$ch),
                   byrow = T)

m1_lwed.freq <- m1_lwed_capHist.r2Proc$data$freq
m1_lwed.group <- m1_lwed_capHist.r2Proc$data$sex
...
m1_lwedF_test1 ## p = 0.994 >> fit CJS model
...
# 1) process data
rmark_m1_lwed.cjs.proc <- RMark::process.data(m1_lwed_capHist,
                                             begin.time = 2010,
                                             model = "CJS",
                                             groups = "sex",
                                             time.intervals = m1_lwed_capIntervals)
...
# 2) design data
rmark_m1_lwed.cjs.ddl <- RMark::make.design.data(rmark_m1_lwed.cjs.proc)

rmark_m1_lwed.cjs.ddl$Phi
rmark_m1_lwed.cjs.ddl$p

# 3) build fxn to create models
rmark_m1_fit.cjs.models <- function(dataProc, dataDesign) {
...
# run the models
rmark_m1_lwed.cjs.models <- rmark_m1_fit.cjs.models(rmark_m1_lwed.cjs.proc, rmark_m1_lwed.cjs.ddl)
...
View(rmark_m1_lwed.cjs.models$model.table)
```
