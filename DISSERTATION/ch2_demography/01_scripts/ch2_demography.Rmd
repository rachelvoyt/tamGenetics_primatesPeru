...
So in package 'marked' there's function "mscjs()" that will compute MLEs for a Multi-state Cormack-Jolly-Seber open population capture-recapture model for processed dataframe x with user specified formulas in parameters that create list of design matrices dml. This function can be called directly but is most easily called from crm that sets up needed arguments. 

OR-- might want mvmscjs() - fitting fxn for multivariate multistate CJS with uncertainty models
--constructed as a hidden Markov model as described in Johnson et al. 2015 

### data

Using package 'marked' (make sure RMark is detached first! too many overlapping function names)
...
Allows adding variables that depend on time (e.g., seasonal changes, weather, catastrophic events)
...
capHist_basta <- splitCH(x = "ch", data = capHist_2009to2019, prefix = "year") %>%
  select(-ch) %>%
  merge(., birthAssignments, by = "animalID", all = T) %>%
  merge(., deathAssignments, by = "animalID", all = T) %>%
  dplyr::rename("birthYear" = "birthYear_est",
                "deathYear" = "deathYear_est") %>%
  relocate(animalID, birthYear, deathYear) %>%
  relocate(-sex)

lwed_capHist_basta <- capHist_basta %>%
  filter(species == "LWED") %>%
  select(-species)

simp_capHist_basta <- capHist_basta %>%
  filter(species == "SIMP") %>%
  select(-species)
...
DataCheck(simp_capHist_basta, dataType = "CMR", studyStart = 2009, studyEnd = 2019)
...
lwed_multibasta <- MultiBaSTA(object = lwed_data_basta,
                              studyStart = 2009,
                              studyEnd = 2019)
