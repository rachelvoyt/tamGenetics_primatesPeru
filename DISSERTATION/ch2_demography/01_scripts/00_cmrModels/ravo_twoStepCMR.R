...
library(rjags) # run JAGS models from R
library(jagsUI) # interface for JAGS to simplify model fitting
library(mcmc) # functions for working with Markov Chain Monte Carlo output

### some JAGS related things that are constant across iterations
## for Two-step
model.file = "./DISSERTATION/ch2_demography/01_scripts/00_cmrModels/ravo_twoStepCR.txt"
params <- c('p.g','psi','p.ind','lam','Ngroups','Ntotal')

## for one-step
model2 <- "./DISSERTATION/ch2_demography/01_scripts/00_cmrModels/ravo_oneStepCR.txt"
params2 <- c('p','psi.i','N')

## set number of iterations for model fitting process
n.iter = 1000
...
## set options for group detection probabilities
P.long <- c(0.3, 0.5, 0.7, 0.9) 

## set individual detection probability
p < -0.7 

## loop through each group detection probability in P.long
for (xy in 1:length(P.long)){
  
  # current group detection prob being tested
  P <- P.long[xy]
  
  # path to dataset
  path <- paste(getwd(), "/TwostepSim_pg", P*10, "_pi", p*10, sep="")
  
  # path to specific data file
  ...
  # inner loop for iterations n.iter
