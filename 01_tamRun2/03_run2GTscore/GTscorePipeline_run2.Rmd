...
sf_p1 <- sf_original_md %>%
  ...
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")
View(sf_p1) # 109 samples; success
...
We can then use that primer-probe file in combination with a file containing the loci that passed and which primer pools they belong to (file created by combining platePrimers_toRepool.csv & oPools_toReorder_142oligos.csv (created in a separate script) and adding primerPool column).

Primer pool splits are as follows:
...
I also found it helpful to create a "master" locus summary file - this way, we have a single file for all loci that provides performance metrics that are specific to that locus's sample subset. This file includes data from both the locus summaries and single SNP summaries created earlier.

Note that I've arranged it such that...

-   values for LWED individual ID loci reflect performance with LWED individuals only
-   values for SIMP individual ID loci reflect performance with SIMP individuals only
-   all other loci reflect performance with LWED and SIMP individuals combined

Note also that all loci appear on the list twice - one set of summary values reflecting the locus's performance in Primer Pool 1, and one set of summary values reflecting its performance in the split pools (either Primer Pool 2 or 3).
...
#### 7.2.2 Pool 1: Combine AmpliconReadCounter individual summary with GTscore sample summary data
...
#### 7.2.3 Pool 2: Combine AmpliconReadCounter individual summary data with GTscore sample summary
...
#### 7.2.4 Pool 3: Combine AmpliconReadCounter individual summary data with GTscore sample summary
