...
write.csv(sampleRef, "./project_data/master_sampleInfo.csv", row.names = F)
```

## Add birth/death data
...
```{r}
sampleRef2 <- read.csv("./project_data/master_sampleInfo.csv") %>%
  merge(., birthData[, c("animalID", "birthDate_est", "birthYear_est")], by = "animalID", all.x = T) %>%
  ...
  merge(., deathData[, c("animalID", "deathYear_est")], by = "animalID", all.x = T) %>%
  relocate(-c(birthDate_est, birthYear_est, natalGroup, deathYear_est)) %>%
  distinct() %>%
  mutate(species = toupper(species)) %>%
  distinct()

write.csv(sampleRef2, "./project_data/master_sampleInfo_v2.csv", row.names = F)
```
