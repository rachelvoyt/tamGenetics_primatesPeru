...
#--------------------------------------------------------------------------------
# NOTE: RV modified lines 310-314 & 325-329 from original script. Matched & unmatched reads (if desired) will now be output to subdirectories within the specified outDir.
...
foreach my$seqFile (@sequenceFiles){
    ...
    if ($printDiscarded) {
        ...
        if ($outDir) {
            # Create the "unmatchedReads" folder within the specified outDir
            my $unmatchedReadDir = "$outDir/unmatchedReads";
            mkdir $unmatchedReadDir unless -e $unmatchedReadDir;
            # Update the path to include the "unmatchedReads" folder
            $unmatchedReadFile = "$unmatchedReadDir/$sampleID\_unmatchedReads.txt";
        }
    ...
    if ($printMatched) {
        ...
        if ($outDir) {
            # Create the "matchedReads" folder within the specified outDir
            my $matchedReadDir = "$outDir/matchedReads";
            mkdir $matchedReadDir unless -e $matchedReadDir;
            # Update the path to include the "matchedReads" folder
            $matchedReadFile = "$matchedReadDir/$sampleID\_matchedReads.txt";
        }
