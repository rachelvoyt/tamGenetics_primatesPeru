# NOTE: RV modified lines 618 & 619 to add an option for "\\[ATGC\\]"="N" to replace brackets in probe (option wasn't present in original script); also modified line 653 to direct script to directory for formatMSAalignments.pl
...
exportRubias<-function(polygenResults,locusTable,sampleMetaData=NULL,sampleWhitelist=NULL,locusWhitelist=NULL,sampleBlacklist=NULL,locusBlacklist=NULL,filename="polygenResults_rubias.txt"){
  ...
  #format column names
  colnames(splitGenotypes)<-str_replace_all(colnames(splitGenotypes),c(".X1"="",".X2"=".1")) # RV updated 8-Dec-2022 from c(".X1",".X2"),c("",".1")
...
alignMatchedSeqs<-function(referenceSeqs=NULL,primerProbes,matchedReads,minReads=1,maxAlignedSeqs=100,type="primerProbe",saveDir=""){
  ...
  for (x in 1:dim(loci)[1]){
    ...
    if(max(uniqueLocusReads$Count>=minReads)){
      ...
      formatMSAcmd<-paste("perl ./GTscore_sourceScripts/formatMSAalignments.pl", dfOut, sep=" ")
      system(formatMSAcmd)
