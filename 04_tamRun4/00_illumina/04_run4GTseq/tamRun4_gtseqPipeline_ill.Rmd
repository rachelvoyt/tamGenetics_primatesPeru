...
get_genoRate <- function(df, lociSet) {
  
  temp <- df %>%
    select(c(sampleIDlociSet$locus)) %>%
...
master_sampleSum <- gtseq_readSums %>%
  ...
  dplyr::rename("sampleID" = "Sample",
                "otReads_fullSet" = "On.Target.Reads") %>%
  mutate(
    sampleID = gsub("-", "\\.", sampleID)
  ) %>%
  mutate_at(vars(loci_fullSet$locus), as.numeric) %>%
  merge(., tamRun4_md[, c("sampleID_ill", "sampleID_ill_md")], by.x = "sampleID", by.y = "sampleID_ill") %>%
  relocate(sampleID, sampleID_ill_md) %>%
  ...
  relocate(c(otReads_dualSet, otReads_lwedSet, otReads_simpSet, otReads_lwedSpec, otReads_simpSpec), .after = otReads_fullSet) %>%
