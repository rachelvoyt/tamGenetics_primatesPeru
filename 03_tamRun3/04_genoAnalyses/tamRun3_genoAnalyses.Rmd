...
included columns with sample names in previous runs if the same samples were run in tamRun1 or tamRun2 so that I can compare genotypes when possible.
...
NOTES
- Locus summaries are based on "cat_" samples (runs a and b combined)
- Sample summaries include a, b, and cat samples
...
sampleOverview_tamRun3 <- matrix(c(tamRun3_lwedHairCount,
  ...
  rowwise() %>%
  mutate(sum = sum(LWED, SIMP, Negatives, na.rm = T)) %>%
  `rownames<-`(., c("totalHair", "hair_F", "hair_M", "totalBlood", "blood_F", "blood_M", "sum"))
```
...
```{r}
sampleSum_blood_cat <- sampleSum_blood %>%
  filter(str_detect(sampleFile, "cat_"))

metrics_blood <- matrix(c(length(sampleSum_blood_cat$sampleFile),
                        median(sampleSum_blood_cat$GenotypeRate),
                        min(sampleSum_blood_cat$GenotypeRate),
                        max(sampleSum_blood_cat$GenotypeRate),
                        sum(sampleSum_blood_cat$GenotypeRate == 0)),
                        nrow = 5,
                        ncol = 1)
```

### 2.1.2 Plot geno rates
...
```{r}
sampleSum_hair_cat <- sampleSum_hair %>%
  filter(str_detect(sampleFile, "cat_"))

metrics_hair <- matrix(c(length(sampleSum_hair_cat$sampleFile),
                        median(sampleSum_hair_cat$GenotypeRate),
                        min(sampleSum_hair_cat$GenotypeRate),
                        max(sampleSum_hair_cat$GenotypeRate),
                        sum(sampleSum_hair_cat$GenotypeRate == 0)),
                        nrow = 5,
                        ncol = 1)
```

### 2.2.2 Plot geno rates
...
### 2.2.3 Geno rate by year

Hair sample genotype rate differs significantly by the year the sample was collected.
...
speciesCount_df <- genos_species_md %>%
  ...
  mutate(propAssigned = totalAssigned/(438 - 9))
speciesCount_df
```
...
genos_sex_a <- genos_tamRun3 %>%
  ...
  mutate(sexAssigned_a = ifelse(propF == 1, "F", 
                              ifelse(propM == 1, "M", 
  ...
  # PRIMER POOL 3
  mutate(propF_pp3 = (rowSums(. == "F")/totalGenos_sex_pp3)) %>%
  mutate(propM_pp3 = (rowSums(. == "M")/totalGenos_sex_pp3)) %>%
  mutate(propMismatch_sex = (1 - abs(propF - propM))) %>%
  mutate(sexAssigned_a_pp3 = ifelse(propF == 1, "F", 
                              ifelse(propM == 1, "M", 
                                     ifelse(propF < 1 & propF > 0, "MIX", NA)))) %>%
...
#  mutate(sumF_pp2 = rowSums(.[grep(c('SEXID_SIMP_197', 'SEXID_SIMP_198', 'SEXID_200', 'SEXID_SIMP_203', 'SEXID_SIMP_218', 'SEXID_219'), names(.))] == "F")) %>%
  rownames_to_column("sampleID") %>%
  relocate(c(sexAssigned_a, totalGenos_sex, propMismatch_sex, propF, propM, propGenos_sex), .after = sampleID) %>%
  relocate(c(SEXID_SIMP_197, SEXID_SIMP_198, SEXID_200, SEXID_SIMP_203, SEXID_SIMP_218, SEXID_219, SEXID_222), .after = propGenos_sex)

genos_sex_b <- genos_tamRun3 %>%
 ...
 # mutate(totalGenos_sex_pp3 = (rowSums(across(matches(loci_sex_pp3)) == "F")) +
  ...
  mutate(propM = (rowSums(. == "M")/totalGenos_sex)) %>%
  mutate(propMismatch_sex = (1 - abs(propF - propM))) %>%
  mutate(sexAssigned_b = ifelse(propF == 1, "F", 
                              ifelse(propM == 1, "M", 
                                     ifelse(propF < 1 & propF > 0, "MIX", NA)))) %>%
  mutate(sumF_pp2 = rowSums(across(matches(loci_sex_pp2)) == "F")) %>%
  mutate(sumM_pp2 = rowSums(across(matches(loci_sex_pp2)) == "M")) %>%
...
#  mutate(sumF_pp2 = rowSums(.[grep(c('SEXID_SIMP_197', 'SEXID_SIMP_198', 'SEXID_200', 'SEXID_SIMP_203', 'SEXID_SIMP_218', 'SEXID_219'), names(.))] == "F")) %>%
  ...
  relocate(c(sexAssigned_b, totalGenos_sex, propMismatch_sex, propF, propM, propGenos_sex), .after = sampleID) %>%
  relocate(c(SEXID_SIMP_197, SEXID_SIMP_198, SEXID_200, SEXID_SIMP_203, SEXID_SIMP_218, SEXID_219, SEXID_222), .after = propGenos_sex)

genos_sex_cat <- genos_tamRun3 %>%
 ...
 # mutate(totalGenos_sex_pp3 = (rowSums(across(matches(loci_sex_pp3)) == "F")) +
  ...
  mutate(propM = (rowSums(. == "M")/totalGenos_sex)) %>%
  mutate(propMismatch_sex = (1 - abs(propF - propM))) %>%
  mutate(sexAssigned_cat = ifelse(propF == 1, "F", 
                              ifelse(propM == 1, "M", 
                                     ifelse(propF < 1 & propF > 0, "MIX", NA)))) %>%
  mutate(sumF_pp2 = rowSums(across(matches(loci_sex_pp2)) == "F")) %>%
  mutate(sumM_pp2 = rowSums(across(matches(loci_sex_pp2)) == "M")) %>%
...
#  mutate(sumF_pp2 = rowSums(.[grep(c('SEXID_SIMP_197', 'SEXID_SIMP_198', 'SEXID_200', 'SEXID_SIMP_203', 'SEXID_SIMP_218', 'SEXID_219'), names(.))] == "F")) %>%
  ...
  relocate(c(sexAssigned_cat, totalGenos_sex, propMismatch_sex, propF, propM, propGenos_sex), .after = sampleID) %>%
  relocate(c(SEXID_SIMP_197, SEXID_SIMP_198, SEXID_200, SEXID_SIMP_203, SEXID_SIMP_218, SEXID_219, SEXID_222), .after = propGenos_sex)
```

Comparing a, b, and cat data

```{r}
sexAssigned_abcat <- merge(genos_sex_a[,1:2], genos_sex_b[,1:2], by = "sampleID") %>%
  merge(., genos_sex_cat[,1:2], by = "sampleID") %>%
  rowwise() %>%
  mutate(uniqueSex = n_distinct(c_across(sexAssigned_a:sexAssigned_cat), na.rm = T))
```
...
For this step, I'll be using the data from the "cat" results for all samples.

## 5.1 Species mismatches

### 5.1.1 Plate maps

Below are the tamRun3 plate maps with the species of each sample as per the metadata. Because all species loci are in primer pool 2, hair sample plates are combined as plates 1 & 2, 3 & 4, and the second half of plate 5 (first half was run with primer pool 3, second half with primer pool 2). 
...
condFormat_species_a <- genos_species_md_a %>%
  mutate(
    species = case_when(
...
plates12_species <- matrix(condFormat_species_a[1:96,]$species,
            nrow = 8,
            ncol = 12,
         byrow = T) %>%
  rbind(1:12,.) %>%
  cbind(c("", 1:8),.) %>%
  kable(caption = "Plates 1 & 2 (hair)", escape = FALSE) %>%
  kable_paper(full_width = F)
plates12_species

plates34_species <- matrix(condFormat_species_a[97:192,]$species,
            nrow = 8,
            ncol = 12,
         byrow = T) %>%
  rbind(1:12,.) %>%
  cbind(c("", 1:8),.) %>%
  kable(caption = "Plates 3 & 4 (hair)", escape = FALSE) %>%
  kable_paper(full_width = F)
plates34_species

plate5_species <- matrix(condFormat_species_a[193:240,]$species,
            nrow = 8,
            ncol = 6,
         byrow = T) %>%
  rbind(7:12,.) %>%
  cbind(c("", 1:8),.) %>%
  kable(caption = "Plate 5 (hair)", escape = FALSE) %>%
  kable_paper(full_width = F)
plate5_species

plate6_species <- matrix(condFormat_species_a[241:336,]$species,
            nrow = 8,
            ncol = 12,
         byrow = T) %>%
  rbind(1:12,.) %>%
...
plate7_species <- matrix(condFormat_species_a[337:432,]$species,
            nrow = 8,
            ncol = 12,
         byrow = T) %>%
  rbind(1:12,.) %>%
...
plate8_species <- matrix(condFormat_species_a[433:438,]$species,
            nrow = 6,
            ncol = 1,
         byrow = T) %>%
  cbind(1:6,.) %>%
...
### 5.1.2 Mismatches by sample type
...
### 5.1.3 Mismatched in blood and hair?

One animalID is mismatched in both blood and hair.
...
### 5.1.3 Mismatched in previous runs? 
...
To troubleshoot sex mismatches, I'm just going to look at the hair samples. 
...
condFormat_sex_a <- genos_sex_md_a %>%
  mutate(
    sex = case_when(
      mdMatch_sp3 == "FALSE" ~ cell_spec(sex, background = "red"),
      mdMatch_sp3 == "TRUE" ~ cell_spec(sex, background = "green"),
      is.na(mdMatch_sp3) ~ cell_spec(sex, background = "grey")
    ))

plates12_sex <- matrix(condFormat_sex_a[1:96,]$sex,
            nrow = 8,
            ncol = 12,
         byrow = T) %>%
  rbind(1:12,.) %>%
...
plates34_sex <- matrix(condFormat_sex_a[97:192,]$sex,
            nrow = 8,
            ncol = 12,
         byrow = T) %>%
  rbind(1:12,.) %>%
...
plate5_sex <- matrix(condFormat_sex_a[193:240,]$sex,
            nrow = 8,
            ncol = 6,
         byrow = T) %>%
  rbind(7:12,.) %>%
...
plate6_sex <- matrix(condFormat_sex_a[241:336,]$sex,
            nrow = 8,
            ncol = 12,
         byrow = T) %>%
  rbind(1:12,.) %>%
...
plate7_sex <- matrix(condFormat_sex_a[337:432,]$sex,
            nrow = 8,
            ncol = 12,
         byrow = T) %>%
  rbind(1:12,.) %>%
...
plate8_sex <- matrix(condFormat_sex_a[433:438,]$sex,
            nrow = 6,
            ncol = 1,
         byrow = T) %>%
  cbind(1:6,.) %>%
...
Create comparison dataframe - note that this only includes loci/sample combinations for which a genotype exists for all runs

```{r}
tamRun3_genos_abcat <- merge(genos_tamRun3al, genos_tamRun3bl, by = c("locus", "sampleID")) %>%
  merge(., genos_tamRun3catl, by = c("locus", "sampleID")) %>%
  rowwise() %>%
  mutate(uniqueGenos = n_distinct(c_across(geno_3a:geno_3cat), na.rm = T))
```

tables

```{r}
metrics_3a_vs_b <- c(length(tamRun3a_vs_b$locus),
                     length(tamRun3a_vs_b$locus[tamRun3a_vs_b$match_3ab %in% "FALSE"]))

tamRun3ab_mismatch <- tamRun3a_vs_b %>%
  filter(match_3ab == "FALSE") %>%
  group_by(locus) %>%
  summarise(n())

tamRun3acat_mismatch <- tamRun3a_vs_cat %>%
  filter(match_3acat == "FALSE") %>%
  group_by(locus) %>%
  summarise(n())

tamRun3bcat_mismatch <- tamRun3b_vs_cat %>%
  filter(match_3bcat == "FALSE") %>%
  group_by(locus) %>%
  summarise(n())
```
...
```{r}
tamRun3_genos_abcat_sp <- tamRun3_genos_abcat %>%
  filter(grepl("SPECIESID", locus))
```
...
```{r}
tamRun3_genos_abcat_sex <- tamRun3_genos_abcat %>%
  filter(grepl("SEXID", locus))
```
