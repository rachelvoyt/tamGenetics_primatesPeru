...
```{r}
# Full set of metadata (latest version from metadataReconciliation)
md_tamRun5_v5 <- read.csv("./metadataReconciliation/tamRun5_metadata_v5.csv")

# Metadata subsets by sample type
md_hairSamples <- md_tamRun5_v5 %>%
  filter(sampleType == "hair") %>%
  # remove tamRunt.264 (animalID 200 dup)
  filter(sampleID != "tamRun5.264")

md_bloodSamples <- md_tamRun5_v5 %>%
  filter(sampleType == "blood") %>%
  # ditch bloodDupes (based on geno success, just checked it manually but should probably do it via script...)
  filter(sampleID != "tamRun5.073") %>%
  filter(sampleID != "tamRun5.132") %>%
  filter(sampleID != "tamRun5.161") %>%
  filter(sampleID != "tamRun5.053") %>%
  filter(sampleID != "tamRun5.054") %>%
  filter(sampleID != "tamRun5.031") %>%
  filter(sampleID != "tamRun5.168")
...
I created the list of twin pairs in tamGenetics_paper3_dataOrganization.Rmd by locating juveniles that were captured in the same group within the same year. In two cases, three juveniles were present; for now, I'm keeping these entries, but removing one of the individuals - **will need to revisit this though.**

It's also possible that there are additional twin pairs that were not captured as juveniles, but I'll be saving these for future analyses as well. 

```{r}
twinList_tamRun5_file <- read.csv("./paper3_demographics/dataOrganization/twinList_tamRun5_6June2024.csv")

twinPairs_tamRun5 <- twinList_tamRun5_file %>%
  # add twinID
  mutate(twinID = row_number()) %>%
  relocate(twinID) %>%
  select(twinID, species, twin1, twin2) %>%
  # add hair sample ID & species
  merge(., md_hairSamples[, c("animalID", "sampleID")], by.x = "twin1", by.y = "animalID") %>%
...
```{r}
bloodReads_allTwins_0x <- twinPairs_tamRun5_l %>%
  select(-species, -sampleID_hair) %>%
  merge(., bloodReads, by.x = "sampleID_blood", by.y = "row.names", all.x = T) %>%
  select(-sampleID_blood) %>%
  relocate(c(animalID, twinID)) %>%
