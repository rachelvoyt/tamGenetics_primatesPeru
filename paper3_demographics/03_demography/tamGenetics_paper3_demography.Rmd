...
Latest version of capData_byIndiv (from
tamGenetics_paper3_dataOrganization) 1) filter to first capture for each
...
# species subsets
capData_byIndiv_lwed <- capData_byIndiv_v5 %>%
  filter(species == "LWED")
capData_byIndiv_simp <- capData_byIndiv_v5 %>%
  filter(species == "SIMP")
...
Based on capData, we have n = 36 likely twin pairs in which both have samples in tamRun5. Of these, n = 25 pairs have both blood and hair samples (and thus can be used for chimerism analyses). Within this set, n = 2 sets have three juveniles vs. two. 
...
capData_twinList_tamRun5 <- capData_twinList %>%
  ...
  merge(., capData_byIndiv_firstEntry[, c("animalID", "species")], by.x = "twin1", by.y = "animalID") %>%
  relocate(species, .after = captureDate) %>%
  relocate(twin1, .before = twin2)

# n = 36 pairs where both have samples in tamRun5
capData_twinList_tamRun5 %>%
  filter(!is.na(twin2_samples)) %>%
  nrow()

# n = 27 pairs where both have blood samples
capData_twinList_tamRun5 %>%
  filter(!is.na(twin2_samples)) %>%
...
# of the 25, have n = 13 LWED pairs & n = 12 SIMP pairs
capData_twinList_tamRun5 %>%
  filter(twin1_samples == "both") %>%
  filter(twin2_samples == "both") %>%
  #filter(species == "LWED") %>%
...
```{r}
write.csv(capData_twinList_tamRun5, "./paper3_demographics/dataOrganization/twinList_tamRun5_6June2024.csv", row.names = F)
```
