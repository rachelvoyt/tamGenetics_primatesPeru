...
(Quantitative analysis of connectivity in populations of a semi‐aquatic
...
How to differentiate b/t parent-offspring vs. sibling relationship when both individuals in a dyad were captured as adults/subadults? 
...
-   LWED
    -   n_total = 137 (w/one duplicate indiv) (n_f = 64, n_m = 71)
-   SIMP
    -   n_total = 99 (n_f = 47, n_m = 52)
...
md_tamRun5_bloodSamples <- md_tamRun5_v5 %>%
  filter(sampleType == "blood")

md_tamRun5_hairSamples <- md_tamRun5_v5 %>%
  filter(sampleType == "hair")

# tamRun4
md_tamRun4 <- read.csv("./04_tamRun4/illONT_tamRun4_metadata.csv")

# quick check to make sure md_tamRun4 has the updated metadata -- looks good (one extra row for env_301 for animalID 91/34, but not an issue for tamRun4)
temp <- md_tamRun4 %>%
  filter(sampleType %in% c("blood", "hair")) %>%
  select(sampleID_ill, sampleID_unique, sampleName) %>%
  merge(., md_tamRun5_v5[, c("sampleID_unique", "sampleName")], by = "sampleName", all.x = T) %>%
  dplyr::rename("sampleID_unique4" = "sampleID_unique.x",
                "sampleID_unique5" = "sampleID_unique.y") %>%
  mutate(
    match = case_when(
      sampleID_unique4 == sampleID_unique5 ~ "yes",
      .default = "no"
    )
  )
```

Below I'm creating LWED and SIMP sample lists that include their
sampleID as well as the sampleID that I'm using for FRANz, which
includes both their animalID and sequencing ID

```{r}
# LWED samples
md_tamRun5_hairSamples_lwed <- md_tamRun5_hairSamples %>%
  filter(species == "LWED")

sampleList_lwedHair <- md_tamRun5_hairSamples_lwed %>%
  select(c("sampleID_unique", "sampleID_franz", "sampleID"))

sampleList_lwedBlood <- md_tamRun5_bloodSamples %>%
  filter(species == "LWED") %>%
  select(c("sampleID_unique", "sampleID_franz", "sampleID"))

# SIMP samples
md_tamRun5_hairSamples_simp <- md_tamRun5_hairSamples %>%
  filter(species == "SIMP")

sampleList_simpHair <- md_tamRun5_hairSamples_simp %>%
  select(c("sampleID_unique", "sampleID_franz", "sampleID"))

sampleList_simpBlood <- md_tamRun5_bloodSamples %>%
  filter(species == "SIMP") %>%
  select(c("sampleID_unique", "sampleID_franz", "sampleID"))
```
...
From 2009 to 2019, we have the following: - LWED - n = 145 total
captured - n_f = 68 - n_m = 77 - SIMP - n = 102 total captured - n_f =
48 - n_m = 54

```{r}
capData_byIndiv_v5 <- read.csv("./paper3_demographics/dataOrganization/captureData_byIndividual_v5.csv") %>%
  mutate(
...
# 2009 to 2019 only
capData_2009to2019_lwed <- capData_byIndiv_v5 %>%
  # up to 2019 only
  filter(rowID <= 613) %>%
  filter(species == "LWED")

capData_2009to2019_simp <- capData_byIndiv_v5 %>%
  # up to 2019 only
  filter(rowID <= 613) %>%
  filter(species == "SIMP")
```
...
Load in latest birthDeath file from tamGenetics_paper3_dataOrganization

```{r}
birthYear_capData_tamRun5_v3 %>%
  filter(animalID %in% md_tamRun5_hairSamples_simp$animalID) %>%
...
```{r}
birthYear_capData_tamRun5_v3 <- read.csv("./paper3_demographics/dataOrganization/birthAssignments_capData_tamRun5_v3.csv")

birthYear_knownOnly <- birthYear_capData_tamRun5_v3 %>%
  select(animalID, birthYear_est) %>%
  na.omit() %>%
  distinct()

deathYear_capData_tamRun5_v1 <- read.csv("./paper3_demographics/dataOrganization/deathAssignments_capData_tamRun5_v1.csv")
```
...
genos0x_tamRun5_lwedHair_indidOnly <- genos0x_tamRun5 %>%
  filter(rownames(.) %in% sampleList_lwedHair$sampleID) %>%
  select(all_of(lociList_lwed))

genos0x_tamRun5_simpHair_indidOnly <- genos0x_tamRun5 %>%
  filter(rownames(.) %in% sampleList_simpHair$sampleID) %>%
  select(all_of(lociList_simp))
```
...
```{r}
capData_parity <- capData_byIndiv_v5 %>%
  filter(sex == "F") %>%
...
capData_parity_firstEntry <- capData_parity[match(unique(capData_parity$animalID), capData_parity$animalID),] %>%
  mutate(
    year_parityStart = str_sub(captureDate, 1, 4),
    year_parityStart = as.numeric(year_parityStart) - 1 # since would've conceived year prior
  )
```
...
concieved prior to 2 years old
...
5)  catchability is same for all indivs (marked and unmakred) at each
...
0.78 - prob of entry = 0.07 each capture event - number of unmarked
indiv is about 8, making the super populat \~7 + 145 marked indiv =
\~152
...
-   LWED
    -   N_est = 152 in superpopulation (based on JS model above)
    -   n_capData = 145 marked individuals (n_fem = 68, n_male = 77)
    -   n_hair = 138 (females = 64; males = 74)
-   SIMP
    -   N_est = 108 in superpopulation (based on JS model above)
    -   n_capData = 102 (n_fem = 48, n_male = 54)
    -   n_hair = 99 (females = 47, males = 52)

Probably not the right way to go about this, but for Nf/Nm estimates I'm
going to 1) calculate the proportion of females/males and 2) multiple
that by N_est to get the estimated total number of mothers/fathers in
the population for FRANz

```{r}
# LWED
(77/(68+77))*152 # est Nf (Nfathers) = 81
(68/(68+77))*152 # est Nm (Nmothers) = 71

# SIMP
(54/(48+54))*108 # est Nfathers = 57
(48/(48+54))*108 # est Nmothers = 51
```
...
Fullsib relationships are very informative and reduce the candidate parents tremendously. FRANz can identify highly probable fullsibs in t he data with the --fullsibtest option. If it is very unlikely that your data contains many fullsibs, you should not turn this on. False positives can decrease the accuracy of the reconstruction. As we have already said, true positives can greatly enhance the accuracy, but if there are no fullsibs, you can only lose.

This feature is still experimental and the default parameters are very arbitrary. The number of false negatives will be high, as this feature is designed to only support the parentage inference. If fullsib inference is your main interest, then you have to set reasonable parameters manually.

Important: --Nmax and --fullsib test together will currently produce biased estimates of N when many fullsibs are detected. Use --N instead or compare the influence of --fullsibtest on the estimated sampling rates.

**--fullsibtest** - Detect siblings. This turns our fullsib heuristic as described in [RSK09] on or off (default --nofull-sibtest). Please read the section FULLSIB before turning this on!

**-- fullsibparental** - Detect fullsib also in the parental generation?

**--fullsibpvth** - The p-Value threshold of the sibling filter. For 0, 1 or 2 common "compatible parental genotypes". Such sampled genotypes are compatible to an offspring genotype according the Mendelian laws and the typing error rate. Default is 0.001, 0.001, 0.05. This means that if individual A and B have no common compatible parental genotypes, a threshold of 0.001 is used. The idea behind these different thresholds is that if A and B have a common compatible parent pair, this is an additional hint that A and B are fullsibs. So we should use a less conservative p-Value threshold (0.05).

**--fullsibH0** - Defines the null hypotheses (PO,HS,U) and their prior probabilities. Examples:
...
```{bash}
FRANz --femrepro 1:15 --malerepro 1:15 --Nf 81 --Nm 71 --typingerror 0.01 --fullsibtest --pout "lwed.er01_" --siblingsout "lwed.er01_" --pedigreeout "lwed.er01_" genos_lwedHair_franz.dat
...
```{r}
# LWED
genos0x_lwed_forFRANz <- names(genos0x_tamRun5_lwedHair_indidOnly) %>%
  map_dfc(~ genos0x_tamRun5_lwedHair_indidOnly %>%
            select(all_of(.x)) %>%
            separate(.x,
                     into = paste0(.x, c("a", "b")),
                     sep = ",")
...
# SIMP
genos0x_simp_forFRANz <- names(genos0x_tamRun5_simpHair_indidOnly) %>%
  map_dfc(~ genos0x_tamRun5_simpHair_indidOnly %>%
            select(all_of(.x)) %>%
            separate(.x,
                     into = paste0(.x, c("a", "b")),
                     sep = ",")
...
# typingerror 0.01
FRANz --femrepro 1:15 --malerepro 1:15 --Nf 81 --Nm 71 --typingerror 0.01 --fullsibtest --pout "lwed_er0.01_parentage" --siblingsout "lwed_er0.01_siblings" --pedigreeout "lwed_er0.01_pedigree" genos0x_lwedHair_franz.dat

# typingerror 0.03
FRANz --femrepro 1:15 --malerepro 1:15 --Nf 81 --Nm 71 --typingerror 0.03 --fullsibtest --pout "lwed_er0.03_parentage" --siblingsout "lwed_er0.03_siblings" --pedigreeout "lwed_er0.03_pedigree" genos0x_lwedHair_franz.dat

# typingerror 0.05
FRANz --femrepro 1:15 --malerepro 1:15 --Nf 81 --Nm 71 --typingerror 0.05 --fullsibtest --pout "lwed_er0.05_parentage" --siblingsout "lwed_er0.05_siblings" --pedigreeout "lwed_er0.05_pedigree" genos0x_lwedHair_franz.dat

# typingerror 0.1
FRANz --femrepro 1:15 --malerepro 1:15 --Nf 81 --Nm 71 --typingerror 0.1 --fullsibtest --pout "lwed_er0.1_parentage" --siblingsout "lwed_er0.1_siblings" --pedigreeout "lwed_er0.1_pedigree" genos0x_lwedHair_franz.dat
```
...
# typingerror 0.01
FRANz --femrepro 1:15 --malerepro 1:15 --Nf 57 --Nm 51 --typingerror 0.01 --fullsibtest --pout "simp_er0.01_parentage" --siblingsout "simp_er0.01_siblings" --pedigreeout "simp_er0.01_pedigree" genos0x_simpHair_franz.dat

# typingerror 0.03
FRANz --femrepro 1:15 --malerepro 1:15 --Nf 57 --Nm 51 --typingerror 0.03 --fullsibtest --pout "simp_er0.03_parentage" --siblingsout "simp_er0.03_siblings" --pedigreeout "simp_er0.03_pedigree" genos0x_simpHair_franz.dat

# typingerror 0.05
FRANz --femrepro 1:15 --malerepro 1:15 --Nf 57 --Nm 51 --typingerror 0.05 --fullsibtest --pout "simp_er0.05_parentage" --siblingsout "simp_er0.05_siblings" --pedigreeout "simp_er0.05_pedigree" genos0x_simpHair_franz.dat

# typingerror 0.10
FRANz --femrepro 1:15 --malerepro 1:15 --Nf 57 --Nm 51 --typingerror 0.1 --fullsibtest --pout "simp_er0.10_parentage" --siblingsout "simp_er0.10_siblings" --pedigreeout "simp_er0.10_pedigree" genos0x_simpHair_franz.dat
```
...
# individual results
po_lwed_run1a <- read.csv("./paper3_demographics/franz/lwed/franz_run1/lwed_er0.01_parentage.csv", na.strings = c("", "NA")) %>%
  rownames_to_column("temp") %>%
  `colnames<-`(colnames_parentage) %>%
  mutate(franzRun = "run1a")

po_lwed_run1b <- read.csv("./paper3_demographics/franz/lwed/franz_run1/lwed_er0.03_parentage.csv", na.strings = c("", "NA")) %>%
  rownames_to_column("temp") %>%
  `colnames<-`(colnames_parentage) %>%
  mutate(franzRun = "run1b")

po_lwed_run1c <- read.csv("./paper3_demographics/franz/lwed/franz_run1/lwed_er0.05_parentage.csv", na.strings = c("", "NA")) %>%
  rownames_to_column("temp") %>%
  `colnames<-`(colnames_parentage) %>%
  mutate(franzRun = "run1c")

po_lwed_run1d <- read.csv("./paper3_demographics/franz/lwed/franz_run1/lwed_er0.10_parentage.csv", na.strings = c("", "NA")) %>%
  rownames_to_column("temp") %>%
  `colnames<-`(colnames_parentage) %>%
...
```{r}
# individual dfs
po_simp_run1a <- read.csv("./paper3_demographics/franz/simp/franz_run1/simp_er0.01_parentage.csv", na.strings = c("", "NA")) %>%
  rownames_to_column("temp") %>%
  `colnames<-`(colnames_parentage) %>%
  mutate(franzRun = "run1a")

po_simp_run1b <- read.csv("./paper3_demographics/franz/simp/franz_run1/simp_er0.03_parentage.csv", na.strings = c("", "NA")) %>%
  rownames_to_column("temp") %>%
  `colnames<-`(colnames_parentage) %>%
  mutate(franzRun = "run1b")

po_simp_run1c <- read.csv("./paper3_demographics/franz/simp/franz_run1/simp_er0.05_parentage.csv", na.strings = c("", "NA")) %>%
  rownames_to_column("temp") %>%
  `colnames<-`(colnames_parentage) %>%
  mutate(franzRun = "run1c")

po_simp_run1d <- read.csv("./paper3_demographics/franz/simp/franz_run1/simp_er0.10_parentage.csv", na.strings = c("", "NA")) %>%
  rownames_to_column("temp") %>%
  `colnames<-`(colnames_parentage) %>%
  mutate(franzRun = "run1d")
```
...
42 offspring (animalID 200 has two entries) had maternity assigned in at least one of the run1 sets
...
Below I'm adding the following variables as checks to determine whether it's reasonable to confirm maternity assignments and thus use them to create a pedigree.dat file for franz_run2:
...
I'm considering the maternity assignment as "confirmed" enough to go into the pedigree.dat file if the following are true:

-   totalMat > 1 (more than 1 maternity assigned across run1 typingerror conditions)
...
Use confirmed maternity assignments to create pedigree.dat file for franz_run2
...
species set, and reformat so that columns = loci & rows = samples; remove commas b/t alleles
...
! There are 11 SNPs scored for <50% of individuals
✖ There are 19 individuals scored for <5% of SNPs, these WILL BE IGNORED
✖ In addition, there are 23 individuals scored for <20% of SNPs,it is advised to treat their assignments with caution
ℹ After exclusion, There are  116  individuals and  153  SNPs.

**After exclusion, we have the following to work with for LWED
samples:** - **116 out of 137 LWED samples** - **153 out of 154 SNPs**


**for tamRun5 seqs only**
GenoConvert gives the following warnings:
...
! There are 9 SNPs scored for <50% of individuals
✖ There are 8 individuals scored for <5% of SNPs, these WILL BE IGNORED
✖ In addition, there are 15 individuals scored for <20% of SNPs,it is advised to treat their assignments with caution
ℹ After exclusion, There are  91  individuals and  150  SNPs.
...
couple notes on variables returned in sequoia dupCheck:
-   Mismatch = # of mismatching genotypes (out of commonGenos)
-   SnpdBoth = # of common genotypes
...
dupCheck is saying all 116 indivs are likely dups, but the dup df they're showing isn't very convincing of that

**tamRun5 only**

Duplicate check returns "There were 32 likely duplicate genotypes found, consider removing".

However, only one of these pairs has > 10 commonGenos - looks like its suggesting duplicates based off ratios of matched to common only, without considering the actual number of commonGenos. In this way, GTscore's dupCheck algorithm seems MUCH better.

As it is though, sequoia's dupCheck only returns one truly likely duplicate pair: tamRun.211 and tamRun5.264, which was already a known duplicate.
...
dupCheck is saying 72 indivs are likely dups, but the dup df they're showing isn't very convincing of that

**tamRun5 only**

Duplicate check returns: "There were 3 likely duplicate genotypes found, consider removing"

Again though, there's no more than 10 common genos, so we can't really say that these are actually likely duplicates.
...
tamRun5.209 tamRun5.283 tamRun5.218   1.12       -0.57        1.86
tamRun5.211 tamRun5.283 tamRun5.218   2.15        1.41        4.21
tamRun5.254 tamRun5.330 tamRun5.339   0.88        1.51        3.29
tamRun5.261 NA          tamRun5.344 999.00        1.20      999.00
tamRun5.274 NA          tamRun5.261 999.00        1.86      999.00
tamRun5.295 tamRun5.314 tamRun5.319   1.59       -0.89        3.50
tamRun5.301 tamRun5.314 NA            0.95      999.00      999.00

275           210       27 ((same as franz))
200           210       27 ((same as franz))
196           14        8 ((same as franz))
142           NA        11 ((DIFFERENT))
281           NA        142 ((same as franz, though franz has 2 parents))
