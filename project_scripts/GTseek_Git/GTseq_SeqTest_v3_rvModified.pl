...
# NOTE - RV modified lines 29-35; it seems like maybe nate changed how the primerProbe file was arranged? or just had typos referring to the wrong columns, dunno
...
while (<SEQ>) {
	chomp;
	my @info = split(/,/, $_);
	my $tring = substr $info[1], 0, 14; # used to be info[5]; rv updated
	$fwd_seq{$info[0]} = $tring;
	$fwd_seqAssay{$tring} = $info[0];
	$probe1{$info[0]} = $info[2]; # used to be info[3]; rv updated
	$probe2{$info[0]} = $info[3]; # used to be info[4]; rv updated
	my $p1RC = reverse $info[2]; # used to be info[3]; rv updated
	my $p2RC = reverse $info[3]; # used to be info[4]; rv updated
	$p1RC =~ tr/ACGT][/TGCA[]/;
	$p2RC =~ tr/ACGT][/TGCA[]/;
