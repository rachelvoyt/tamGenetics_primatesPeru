#!/usr/bin/perl
# NOTE - RV modified allele ratio cutoffs to match those specified in GTseq_Genotyper_v2.pl. For whatever reason this version works but that one doesn't, but I need the ratios to be the same for genos.
