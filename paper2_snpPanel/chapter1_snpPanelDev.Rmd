...
get_gtseqGenos_v2 <- function(readCounts, Allele1, Allele2) {
  
  # Function to assign single geno
  assign_geno <- function(rc, Allele1, Allele2) {
    ...
    # Calculate allele1/allele2 ratio; round to 3 places
    a1a2_ratio <- case_when(
      is.na(readSum) ~ NA_real_,
      readSum < 10 ~ NA_real_,
      TRUE ~ round(counts[1]/counts[2], 3)
    )
    
    # assign geno
    geno <- case_when(
      is.na(a1a2_ratio) ~ NA_character_,
      a1a2_ratio >= 10 ~ str_c(Allele1, Allele1, sep = ","),
      a1a2_ratio <= 0.1 ~ str_c(Allele2, Allele2, sep = ","),
      a1a2_ratio <= 5 ~ str_c(Allele1, Allele2, sep = ","),
      TRUE ~ NA_character_
    )
    return(geno) 
  }
  
  # Apply the function to each element in readCounts
  sapply(readCounts, assign_geno, Allele1, Allele2)
}
...
#  separate(readCounts_gs, into = c("a1Counts_gs", "a2Counts_gs"), sep = ",", remove = F) %>%
  ...
  mutate(
    test = get_gtseqGenos_v2(readCounts_gs, Allele1, Allele2)
  )
