...
tamRun4_readCounts_ill_gtseq <- read.delim("./04_tamRun4/00_illumina/04_run4GTseq/ill_gtseq_alleleReads.txt", header = TRUE, row.names = 1, stringsAsFactors = FALSE)
...
```{r}
# allele reads
tamRun4_ill_gtscore_alleleReads_l <- tamRun4_readCounts_ill_gtscore %>%
  rownames_to_column("locus") %>%
  pivot_longer(!locus,
...
tamRun4_ill_gtseq_alleleReads_l <- tamRun4_readCounts_ill_gtseq %>%
  rownames_to_column("locus") %>%
  pivot_longer(!locus,
               names_to = "sampleID_ill",
...
tamRun4_ill_gtseq_genos_l <- tamRun4_genos_ill_gtseq %>%
  rownames_to_column("locus") %>%
  pivot_longer(!locus,
               names_to = "sampleID_ill",
               values_to = "genos_gs") %>%
...
odd discrepancy - in Campbell et al. 2015, they say that genotypes are assigned as follows:

	* allele ratios > 10 = homozygous for allele 1
	* allele ratios < 0.1 = homozygous for allele 2
	* allele ratios b/t 0.2 & 5.0 = heterozygous
	* total read counts < 10 = NA
	* allele ratios b/t 0.1 & 0.2 or 5.0 & 10.0 = NA

However, in GTseq_Genotyper.V3.pl, there's no line accounting for ratios in the range (2, 5]; instead, genotypes are assigned as follows:

-   allele ratios >= 10 = homozygous for allele 1
-   allele ratios <= 0.1 = homozygous for allele 2
-   heterozygotes are assigned to ratios (0.1, 0.2] 
...
## based on GTseq_Genotyper_v3.pl (the one I used for running gtseq pipeline)
get_gtseqGenos_v3 <- function(ratio, Allele1, Allele2) {
...
```{r}
compare_gtseq.score <- tamRun4_ill_gtscore_alleleReads_l %>%
  ...
  # calculate allele ratios for gtscore & gtseq allele reads
  mutate(
  ...
  # assign genos using gtseq ratios + gtseq genotypers v2 & v3
  mutate(
    genos_v2 = get_gtseqGenos_v2(ratio_gs, Allele1, Allele2),
    genos_v3 = get_gtseqGenos_v3(ratio_gs, Allele1, Allele2)
  ) %>%
  ...
  mutate(
    genoMatch_gs.v3 = case_when(
      is.na(genos_gs) & !is.na(genos_v3) ~ "missing_gs",
      !is.na(genos_gs) & is.na(genos_v3) ~ "missing_v3",
      genos_gs == genos_v3 ~ "TRUE",
      genos_gs != genos_v3 ~ "FALSE"
    )
  ) %>%
  mutate(
    genoMatch_v2.v3 = case_when(
      is.na(genos_v2) & !is.na(genos_v3) ~ "missing_v2",
      !is.na(genos_v2) & is.na(genos_v3) ~ "missing_v3",
      genos_v2 == genos_v3 ~ "TRUE",
      genos_v2 != genos_v3 ~ "FALSE"
    )
  ) %>%
  mutate(
  ...
  mutate(
    genoMatch_gr.v3 = case_when(
      is.na(genos_gr) & !is.na(genos_v3) ~ "missing_gr",
      !is.na(genos_gr) & is.na(genos_v3) ~ "missing_v3",
      genos_gr == genos_v3 ~ "TRUE",
      genos_gr != genos_v3 ~ "FALSE"
    )
  ) %>%
  mutate(
  ...
  mutate(
    ...
    callType_v3 = get_callType_gs(a1Counts_gs, a2Counts_gs, genos_v3, Allele1, Allele2)
  )

table(compare_gtseq.score$callType_gr)
table(compare_gtseq.score$genoMatch_gs.v3)
table(compare_gtseq.score$genoMatch_gr.v2)
table(compare_gtseq.score$genoMatch_gr.v3)
```
...
ggplot(compare_gtseq.score, aes(x = as.numeric(a1Counts_gs), y = as.numeric(a2Counts_gs), color = callType_v3, shape = callType_v3)) +
  geom_point() +
  labs(title = "gtseq_v3",
       x = "allele1Counts",
       y = "allele2Counts") +
