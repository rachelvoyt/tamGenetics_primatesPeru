...
temp_0x <- genoCompare_ill_gtscore0x %>%
  group_by(animalID) %>%
  summarise(
    mismatchType2_bf = sum(bf_misType == "type2", na.rm = T),
    mismatchType2_bh = sum(bh_misType == "type2", na.rm = T),
    mismatchType2_fh = sum(fh_misType == "type2", na.rm = T),
...
temp <- genoReadCompare_ill_gtscore10x_geno100 %>%
  merge(., homoHet_key, by.x = c("locus", "blood"), by.y = c("locus", "geno")) %>%
  merge(., homoHet_key, by.x = c("locus", "fecal"), by.y = c("locus", "geno"), suffixes = c("_blood", "_fecal")) %>%
  merge(., homoHet_key, by.x = c("locus", "hair"), by.y = c("locus", "geno")) %>%
  dplyr::rename("callType_hair" = "callType") %>%
  mutate(
    bf_misType = case_when(
      bfMatch == TRUE ~ NA,
...
temp2 <- temp %>%
  filter(bfMatch == FALSE)

table(temp2$bf_misType)
```
