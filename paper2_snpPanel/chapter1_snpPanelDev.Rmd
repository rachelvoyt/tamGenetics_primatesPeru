...
tamRun4_alleleReads_hair <- tamRun4_readCounts_ill_gtscore %>%
  select(tamRun4_hairSamples$sampleID_ill) %>%
  rownames_to_column("locus") %>%
  pivot_longer(!locus,
               names_to = "sampleID_ill",
               values_to = "readCounts_run4") %>%
  merge(., tamRun4_md[, c("sampleID_ill", "animalID")], by = "sampleID_ill")

tamRun4_genos0x_hair <- tamRun4_genos_ill_gtscore0x %>%
  select(tamRun4_hairSamples$sampleID_ill) %>%
  rownames_to_column("locus") %>%
  pivot_longer(!locus,
               names_to = "sampleID_ill",
               values_to = "genos_run4") %>%
  merge(., tamRun4_md[, c("sampleID_ill", "animalID")], by = "sampleID_ill")
```
...
get_gtseqRatio <- function(readCounts) {
  ...
  readSum <- gsubfn("(\\d+),(\\d+)", ~ as.numeric(x) + as.numeric(y), paste(readCounts))
  
  if(as.numeric(readSum) < 10) {
    NA
  } else {
    gsubfn("(\\d+),(\\d+)", ~ as.numeric(x)/as.numeric(y), paste(readCounts))
    }
}
...
genoCompare_baseGenos <- tamRun4_genos0x_hair %>%
  merge(., tamRun5_genos0x_hair, by = c("animalID", "locus"), all.x = T) %>%
  #mutate(across(everything(), na_if, "0")) %>%
  mutate(across(everything(), \(x) na_if(x, "0"))) %>%
  mutate(
    genoMatch = genos_run4 == genos_run5
  ) %>%
  merge(., tamRun4_alleleReads_hair, by = c("animalID", "locus"), all.x = T) %>%
  merge(., tamRun5_alleleReads_hair, by = c("animalID", "locus"), all.x = T) %>%
  select(!starts_with("sampleID")) %>%
  mutate(
    gtRatio_run4 = round(get_gtseqRatio(readCounts_run4), 2),
    gtRatio_run5 = round(get_gtseqRatio(readCounts_run5), 2)
  )
```
