...
knitr::opts_knit$set(root.dir = "./01_run2_fecalHairBlood/03_run2GTscore")
```
...
This pipeline includes all steps needed to analyze MiSeq data output, including how to 1) interleave the fasta sequences, 2) create the necessary files for analysis in GTscore, and 3) run the GTscore pipeline itself.

The GTscore pipeline was originally created by Garrett McKinney (see the associated github [here](https://github.com/gjmckinney/GTscore)). Here, I combine the scripts included in his original "GTscore_pipeline.R" as well as his original "README.md" - there's a good bit of overlap between these files, but a few scripts exist in one but not the other. I've also included separate analyses for each species as well as for each primer pool, since not all samples were run with all loci and not all loci are applicable to both species.

## 1.2 About the data

The sequence data used in this pipeline is from tamRun2, the second sequencing run in this project. This run has a total of **142 samples**, including:
...
Note that Run 2 analyses only include loci that passed the first round of optimization.
...
GTscore also requires Perl as well as the the packages "Algorithm::Combinatorics" and "Excel-Writer-XLSX". Perform the following in terminal to install on Linux. NOTE -- be sure to install both perl and the modules in root. See how to install perl packages [here](http://www.cpan.org/modules/INSTALL.html).
...
Note that I'm using raw (unfiltered) reads for this pipeline. Quality checks are included in the run2Analyses_report.Rmd file.

## 3.1 Extract sequencing results

After downloading the results from Basespace, transfer the zip file to 00_originalSeqResults and extract into 01_run2Seqs (keep the original zip file as a copy in 00_originalSeqResults):
...
## 3.2 Interleave fasta sequences

To prep for the GTscore pipeline, we need to combine reads 1 & 2 for each sample into a single interleaved file & unzip:
...
Step 1: Create general sample file "run2_sampleFiles_original.txt" containing all the names of the interleaved fasta files created above (make this by hand; use terminal to list all file names and create a new txt file)

Step 2: Split the general sample file into i) lwed & simp samples, then ii) full pool & split pool samples

Primer Pool 1 (115 samples total, includes 2\*(5 pcr1 neg + 1 pcr2 neg)) 50 LWED + 5 pcr1 (-) + 1 pcr2 (-) = 56 total 53 SIMP + 5 pcr1 (-) + 1 pcr2 (-) = 59 total

Primer Pool 2 (25 samples total, includes 2\*(1 pc1 neg & 1 pcr2 neg)) 6 LWED + 1 pcr1 neg + 1 pcr2 neg = 8 total 15 SIMP + 1 pcr1 neg + 1 pcr2 neg = 17 total

Primer Pool 3 (13 samples total, includes 2\*(1 pcr2 neg)) 5 LWED + 1 pcr2 (-) = 6 total 6 SIMP + 1 pcr2 (-) = 7 total

### 4.1.1 Read in metadata

```{r}
# Read in metadata
md <- read.csv(here("./01_run2_fecalHairBlood/03_run2GTscore/metadata_tamRun2.csv")) %>%
  filter(included_inRun == "yes") %>%
  select(c(sampleID, species, primerPool))
md$sampleID <- gsub("_", "-", md$sampleID) # adjust formatting; miseq changes "_" to "-" for some reason
length(md$sampleID) # double check there's 142 samples; success
table(md$primerPool)
```

### 4.1.2 Create sample file subsets

Filter run2_sampleFiles to create separate LWED & SIMP for each pool set (full & split)

```{r}
# Read in original list of run2 sample files
sf_original <- read.table(here("./01_run2_fecalHairBlood/03_run2GTscore/run2_sampleFiles_original.txt"))

# Reformat column & attach metadata
sf_original_md <- sf_original %>%
  separate(V1, into = c("x", "y"), sep = 11) %>%
  merge(., md, by.x = "x", by.y = "sampleID")

# Separate sf_original into p1, p2, & p3 (note that the pcr2 (-) is included in all subsets)
sf_p1 <- sf_original_md %>%
  filter(is.na(primerPool) | !primerPool %in% c("2", "3")) %>%
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")
View(sf_p1) # 109 samples

sf_p2 <- sf_original_md %>%
  filter(is.na(primerPool) | !primerPool %in% c("1", "3")) %>%
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")
View(sf_p2) # 23; success

sf_p3 <- sf_original_md %>%
  filter(is.na(primerPool) | !primerPool%in% c("1", "2")) %>%
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")
...
## LWED
sf_LWED_p1 <- sf_original_md %>%
  filter(is.na(primerPool) | !primerPool %in% c("2", "3")) %>%
  filter(is.na(species) | !species == "SIMP") %>%
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")

sf_LWED_p2 <- sf_original_md %>%
  filter(is.na(primerPool) | !primerPool %in% c("1", "3")) %>%
  filter(is.na(species) | !species == "SIMP") %>%
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")

sf_LWED_p3 <- sf_original_md %>%
  filter(is.na(primerPool) | !primerPool %in% c("1", "2")) %>%
  filter(is.na(species) | !species == "SIMP") %>%
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")
...
### SIMP
sf_SIMP_p1 <- sf_original_md %>%
  filter(is.na(primerPool) | !primerPool %in% c("2", "3")) %>%
  filter(is.na(species) | !species == "SFUS") %>%
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")

sf_SIMP_p2 <- sf_original_md %>%
  filter(is.na(primerPool) | !primerPool %in% c("1", "3")) %>%
  filter(is.na(species) | !species == "SFUS") %>%
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")

sf_SIMP_p3 <- sf_original_md %>%
  filter(is.na(primerPool) | !primerPool %in% c("1", "2")) %>%
  filter(is.na(species) | !species == "SFUS") %>%
  select(c(x, y)) %>%
  unite(V1, x, y, sep = "")
...
```{r}
# Files for each primer pool, species combined
write.table(sf_p1, file = here("./01_run2_fecalHairBlood/03_run2GTscore/p1_combined_sampleFiles.txt"), sep = "\t", row.names = F, col.names = F, quote = F)
write.table(sf_p2, file = here("./01_run2_fecalHairBlood/03_run2GTscore/p2_combined_sampleFiles.txt"), sep = "\t", row.names = F, col.names = F, quote = F)
write.table(sf_p3, file = here(".//01_run2_fecalHairBlood/03_run2GTscore/p3_combined_sampleFiles.txt"), sep = "\t", row.names = F, col.names = F, quote = F)

# Files for each primer pool, LWED only
write.table(sf_LWED_p1, file = here("./01_run2_fecalHairBlood/03_run2GTscore/p1_LWED_sampleFiles.txt"), sep = "\t", row.names = F, col.names = F, quote = F)
write.table(sf_LWED_p2, file = here("./01_run2_fecalHairBlood/03_run2GTscore/p2_LWED_sampleFiles.txt"), sep = "\t", row.names = F, col.names = F, quote = F)
write.table(sf_LWED_p3, file = here("./01_run2_fecalHairBlood/03_run2GTscore/p3_LWED_sampleFiles.txt"), sep = "\t", row.names = F, col.names = F, quote = F)

# Files for each primer pool, SIMP only
write.table(sf_SIMP_p1, file = here("./01_run2_fecalHairBlood/03_run2GTscore/p1_SIMP_sampleFiles.txt"), sep = "\t", row.names = F, col.names = F, quote = F)
write.table(sf_SIMP_p2, file = here("./01_run2_fecalHairBlood/03_run2GTscore/p2_SIMP_sampleFiles.txt"), sep = "\t", row.names = F, col.names = F, quote = F)
write.table(sf_SIMP_p3, file = here("./01_run2_fecalHairBlood/03_run2GTscore/p3_SIMP_sampleFiles.txt"), sep = "\t", row.names = F, col.names = F, quote = F)
```
...
We already have our original primerProbeFile from the optimization run; now we need to filter out the loci that didn't pass optimization and create separate files for full pool (p1) and split pool (p2 & p3).

### 4.2.1 Load in original primer-probe files

We first have to recombine the optimization primer-probe files, which were separated into those with and without the bp3 error.
...
pp_original_all <- rbind(pp_original_clean, pp_original_bp3error) %>%
  separate(Locus, into = c("locusName", "bp3Index"), sep = "_") %>%
  separate(bp3Index, into = c("bp3Index", "bp3Rank"), sep = "\\.")
length(pp_original_all$locusName) # 302 loci total; success
```

### 4.2.2 Create primer-probe file subsets

We can then use that primer-probe file in combination with a file containing the loci that passed and which primer pools they belong to (file created by manually combining platePrimers_toRepool.csv & oPools_toReorder_142oligos.csv (created in a separate script) and adding primerPool column). Primer pool splits are as follows:
-   Pool 1: 442 primers (221 loci)
-   Pool 2: 222 primers (111 loci)
-   Pool 3: 220 primers (110 loci)
...
Create primer-probe file subsets
```{r}
# Primer Pool 1
pp_p1 <- pp_original_all %>%
  filter(bp3Index %in% poolSplits$bp3Index) %>%
  unite(Locus, locusName, bp3Index, sep = "_") %>%
  unite(Locus, Locus, bp3Rank, sep = ".") # 221; success
...
pp_p2 <- pp_original_all %>% 
  filter(bp3Index %in% poolSplits$bp3Index[poolSplits$primerPool == "2"]) %>%
  filter(bp3Index %in% poolSplits$bp3Index) %>%
  unite(Locus, locusName, bp3Index, sep = "_") %>%
  unite(Locus, Locus, bp3Rank, sep = ".") # 111; success
...
pp_p3 <- pp_original_all %>% 
  filter(bp3Index %in% poolSplits$bp3Index[poolSplits$primerPool == "3"]) %>%
  filter(bp3Index %in% poolSplits$bp3Index) %>%
  unite(Locus, locusName, bp3Index, sep = "_") %>%
  unite(Locus, Locus, bp3Rank, sep = ".") # 110; success
...
Export files
```{r}
write.table(pp_p1, here("./01_run2_fecalHairBlood/03_run2GTscore/p1_combined_primerProbeFile.txt"), sep = "\t", row.names = F, quote = F)
write.table(pp_p1_LWED, here("./01_run2_fecalHairBlood/03_run2GTscore/p1_LWED_primerProbeFile.txt"), sep = "\t", row.names = F, quote = F)
write.table(pp_p1_SIMP, here("./01_run2_fecalHairBlood/03_run2GTscore/p1_SIMP_primerProbeFile.txt"), sep = "\t", row.names = F, quote = F)

write.table(pp_p2, here("./01_run2_fecalHairBlood/03_run2GTscore/p2_combined_primerProbeFile.txt"), sep = "\t", row.names = F, quote = F)
write.table(pp_p2_LWED, here("./01_run2_fecalHairBlood/03_run2GTscore/p2_LWED_primerProbeFile.txt"), sep = "\t", row.names = F, quote = F)
write.table(pp_p2_SIMP, here("./01_run2_fecalHairBlood/03_run2GTscore/p2_SIMP_primerProbeFile.txt"), sep = "\t", row.names = F, quote = F)

write.table(pp_p3, here("./01_run2_fecalHairBlood/03_run2GTscore/p3_combined_primerProbeFile.txt"), sep = "\t", row.names = F, quote = F)
write.table(pp_p3_LWED, here("./01_run2_fecalHairBlood/03_run2GTscore/p3_LWED_primerProbeFile.txt"), sep = "\t", row.names = F, quote = F)
write.table(pp_p3_SIMP, here("./01_run2_fecalHairBlood/03_run2GTscore/p3_SIMP_primerProbeFile.txt"), sep = "\t", row.names = F, quote = F)
```

# 5 Count reads for amplicons

Once the files are created, we can proceed with the first step in the GTscore pipeline - counting amplicon reads. We'll be doing doing this for the full set of data (all samples, all loci) as well as the various species/primer-pool subsets of the data to allow for more accurate analyses of loci and sample performance:
...
Input flags for this script are: \* --p a tab delimited file containing primer/probe information for each locus \* --files a text file containing a list of .fastq sequence files to count reads from.

Optional flags: \* --prefix optional prefix for output file names \* --inDir option to specify directory containing sequence data \* --inputType fq or fastqgz (defaults to fastqgz) \* --useFullPrimer uses the full primer for counting reads rather than the trimmed primer \* --alleleOrder order of alleles output in locusTable file. Options are original (matches primer-probe file order) or alphabetical (default) \* --printMatched outputs matched reads for each individual \* --printDiscarded outputs discarded reads for each individual
...
#### 7.1.1.1 Single SNP summaries: Pool 1
...
#### 7.1.1.2 Single SNP summaries: Pool 2
...
#### 7.1.1.3 Single SNP summaries: Pool 3
...
#### 7.1.2.4 Create master locus summary file
