...
ggplot(sumReads_perLocus, aes(x=readSum, y=factor(0))) +
  geom_boxplot() +
  labs(title = "Total reads per locus (all samples)", x = "Number of reads", y = "") +
  theme_bw()
...
sumReads_perLocus %>%
  arrange(-readSum) %>%
  kbl(align = "c", col.names = c("Locus", "Total primer-probe reads", "Outlier locus?")) %>%
  kable_material(full_width = F) %>%
...
sumReads_perLocus_failedSamples %>%
  dplyr::select(c(readSum, outlier)) %>%
  na.omit() %>%
  arrange(-readSum) %>%
  kbl(align = "c", col.names = c("Total primer-probe reads", "Outlier locus")) %>%
  kable_material(full_width = F) %>%
  scroll_box(height = "500 px", fixed_thead = list(enabled = T, background = "#363636"))
...
If we look at only our successful samples (\> 50% genotype success), do we see the same loci overamplifying?
...
Which samples are the reads for this locus coming from?
...
grVrpl <- ggplot() +
  geom_point(data=lociPF,aes(x=Primer.Probe.Reads,y=GenotypeRate, color=as.character(primerPool)), size = 3, alpha = 0.5) +
