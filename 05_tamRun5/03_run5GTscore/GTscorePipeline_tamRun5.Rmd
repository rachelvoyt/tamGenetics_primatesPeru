...
source("./project_scripts/GTscore/GTscore_modified.R") # NOTE- added N=ATGC to script for later analyses (not included in original)
source("./project_scripts/ravo_polygen.R") # my version of polygen script
...
For tamRun5b, I had access to the run data but not the fastq files, so I opted to download the run data and regenerate the fastq files by installing and using bcl2fastq following <https://support.parsebiosciences.com/hc/en-us/articles/360058912372-Generating-fastq-files-with-bcl2fastq>.

First I acquired the run ID from the url -- 285748489, then created the fastq files:

```{bash}
# directory
cd /home/rachelvoyt/Documents/UT-Grad/Development/repos/tamGenetics_primatesPeru/05_tamRun5/00_run5b_basespaceData

# download run data
bs run download --id 285748489 -o .

# convert bcl to fastq files
mkdir fastq_files
bcl2fastq -i Data/Intensities/BaseCalls/ --output-dir fastq_files

# got "Too many open files" error -- check limit; increase limit
ulimit -n # 1024
ulimit -n 4096
...
# trim filenames to make them more manageable
cd fastq_files/tamGenetics/
for f in *R1_001.fastq; do g="${f%%_S*}_R1.fastq"; mv "${f}" "${g}"; done
for f in *R2_001.fastq; do g="${f%%_S*}_R2.fastq"; mv "${f}" "${g}"; done

# edit run filenames to be tamRun5b
rename 's/tamRun5/tamRun5b/' *.fastq

# copy files to run5Seqs
cd ./../../../
cp ./00_run5b_basespaceData/fastq_files/tamGenetics/tamRun* 00_run5Seqs
```
...
# adjust - to _ in tamRun5a filenames
rename 's/tamRun5a-/tamRun5a_/' *.fastq

# Create tamRun5ab merged fastq files
for i in tamRun5a_*; do 
    f=${i##*tamRun5a}; 
...
for i in ./00_run5Seqs/*_R1.fastq; do name=$(basename $i _R1.fastq); reformat.sh in=00_run5Seqs/${name}_R1.fastq in2=00_run5Seqs/${name}_R2.fastq out=02_run5Interleaved/${name}.fastq; done
```
...
system2("/home/rachelvoyt/perl5/perlbrew/perls/perl-5.38.0/bin/perl",
        args="./project_scripts/GTscore/AmpliconReadCounter_modified.pl -p ./project_data/primerProbeFile_panelv3_fullSet.txt --files ./05_tamRun5/03_run5GTscore/run5b_sampleFiles_fullSet.txt --inDir ./05_tamRun5/02_run5Interleaved/ --outDir ./05_tamRun5/03_run5GTscore/ --prefix run5b_fullSet_")

system2("/home/rachelvoyt/perl5/perlbrew/perls/perl-5.38.0/bin/perl",
        args="./project_scripts/GTscore/AmpliconReadCounter_modified.pl -p ./project_data/primerProbeFile_panelv3_fullSet.txt --files ./05_tamRun5/03_run5GTscore/run5cat_sampleFiles_fullSet.txt --inDir ./05_tamRun5/02_run5Interleaved/ --outDir ./05_tamRun5/03_run5GTscore/ --prefix run5cat_fullSet_")


## skipped updating below
...
Using a 10x read depth cutoff for genotyping via my modified version of the gtscore script (located in ravo_polygen.R)

```{r}
#load locus table and allele reads file
fullSet_singleSNP_locusTable <- read.delim("./05_tamRun5/03_run5GTscore/run5cat_fullSet_LocusTable_singleSNPs.txt", header = TRUE, stringsAsFactors = FALSE)

fullSet_singleSNP_alleleReads <- read.delim("./05_tamRun5/03_run5GTscore/run5cat_fullSet_AlleleReads_singleSNPs.txt", header = TRUE, row.names = 1, stringsAsFactors = FALSE)

#generate singleSNP genotypes using the polyGen algorithm, adjust "0" formatting
fullSet_polyGenResults <- polyGen_rv(fullSet_singleSNP_locusTable, fullSet_singleSNP_alleleReads, depthCutoff = 10)

# write results
write.table(fullSet_polyGenResults, "./05_tamRun5/03_run5GTscore/run5cat_fullSet_polyGenResults_singleSNP_10x.txt", quote = FALSE, sep = "\t")
```
