...
Loop form for tamRun5 data:

```{bash, eval = F}
cd /home/rachelvoyt/Documents/UT-Grad/Development/repos/tamGenetics_primatesPeru/05_tamRun5/05_run5GTseq

for i in ./../02_run5Interleaved/tamRun5*; do perl GTseq_HashSeqs.pl $i > ./hashSeqs/$(basename "${i/.fastq/.hash}"); done
```
...
write.table(pp_gtseq_seqTest,"./05_tamRun5/05_run5GTseq/primerProbe_gtseq.txt", quote = F, sep = "\t", col.names = F, row.names = F)
```
...
write.table(pp_gtseq_genotyper, "./05_tamRun5/05_run5GTseq/locusInfo_gtseq.csv", col.names = F, row.names = F, sep = ",", quote = F)
```
...
for i in ./../02_run5Interleaved/tamRun5*; do perl GTseq_Genotyper_v3.pl locusInfo_gtseq.csv $i > ./genos/$(basename "${i/.fastq/.genos}"); done
```
