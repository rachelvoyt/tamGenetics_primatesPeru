...
Below is the metadata for samples included in tamRun5. I've also included columns with sample names in previous runs if the same samples were run in tamRun1, tamRun2, or tamRun3 so that I can compare genotypes when possible.
...
sampleSum_gtseq <- read_csv("./05_tamRun5/05_run5GTseq/genos/compiledGenos_gtseq_tamRun5.csv") %>%
  ...
  dplyr::rename_all(funs(make.names(.))) %>%
  mutate(
    GenotypeRate = X.GT/100
  )

sampleSum_blood_gtseq <- sampleSum_gtseq %>%
  filter(Sample %in% bloodSamples$sampleID)

sampleSum_hair_gtseq <- sampleSum_gtseq %>%
  filter(Sample %in% hairSamples$sampleID)
```
...
```{r, echo=FALSE}
genoRate_summary_tamRun5 <- matrix(
  # blood
  c(length(sampleSum_blood$sampleFile),
    sum(sampleSum_blood$GenotypeRate == 0),
    scales::percent(median(sampleSum_blood$GenotypeRate)),
    scales::percent(min(sampleSum_blood$GenotypeRate)),
    scales::percent(max(sampleSum_blood$GenotypeRate)),
    # hair
    length(sampleSum_hair$sampleFile),
    sum(sampleSum_hair$GenotypeRate == 0),
    scales::percent(median(sampleSum_hair$GenotypeRate)),
    scales::percent(min(sampleSum_hair$GenotypeRate)),
    scales::percent(max(sampleSum_hair$GenotypeRate))),
  nrow = 5,
  ncol = 2) %>%
...
genoRate_summary_tamRun5 %>%
  kable(caption = "Overview of genotype success by sample type") %>%
  kable_material(full_width = F) %>%
  scroll_box(fixed_thead = list(enabled = T, background = "#363636"))
```
...
ggplot() + 
  ...
  annotate(x = 0.54, y = 27, label = "Median = 0.64", geom = "label") +
...
ggplot() + 
  ...
  labs(title="Proportion of loci genotyped: Hair samples (n = 237)", x="Proportion of loci genotyped", y="Number of hair samples") +
  theme_bw() + 
  theme(plot.title=element_text(hjust=0.5),plot.subtitle=element_text(hjust=0.5))
```
...
```{r}
summary.aov(aov(GenotypeRate ~ captureYear, data = sampleSum_blood))
```

```{r, echo=FALSE}
ggplot(sampleSum_blood, aes(x = captureYear, y = GenotypeRate, group = captureYear)) +
...
```{r}
summary.aov(aov(GenotypeRate ~ captureYear, data = sampleSum_hair))
```

```{r, echo=FALSE}
ggplot(sampleSum_hair, aes(x = captureYear, y = GenotypeRate, group = captureYear)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(color=factor(captureYear))) +
...
```{r, echo=FALSE}
ggplot(sampleSum_hair, aes(x = xtnYear, y = GenotypeRate, group = xtnYear)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(aes(color=factor(captureYear))) +
...
```{r}
duptest <- read.table("./05_tamRun5/03_run5GTscore/run3_polyGenResults_dupTest.txt", header = T) 
