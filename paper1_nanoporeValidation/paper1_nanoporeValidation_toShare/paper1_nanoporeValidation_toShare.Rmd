---
title: "Nanopore validation paper: Tamarin pipeline"
author: "Rachel Voyt"
date: "`r Sys.Date()`"
output:
  html_document:
...
knitr::opts_chunk$set(echo = TRUE, root.dir = "/home/rachelvoyt/Documents/UT-Grad/Development/repos/tamGenetics_primatesPeru/paper1_nanoporeValidation/paper1_nanoporeValidation_toShare/")
```
...
vs. Nanopore** genotype results for **10 tamarin blood samples** at **74
SNP loci (740 genotypes total)**.
...
including four saddleback tamarins (*Leontocebus weddelli*; n = 2
females, n = 2 males) and six emperor tamarins (*Saguinus imperator*; n
= 2 females, n = 4 males).

```{r, echo=FALSE}
md <- read.csv("metadata_paper1_nanoporeValidation.csv")
```
...
Our loci dataset is composed of **74 dual-species SNPs**, including SNPs
used for the identification of individuals (INDID; n = 64), sex (SEXID;
n = 3), and species (SPECIESID; n = 7). All 74 loci are informative for
both tamarin species.

```{r, echo=FALSE}
loci <- read.table("primerProbeFile.txt", header = T)
...
# illumina dataset
system2("perl",
        args="./scripts/AmpliconReadCounter_modified.pl -p primerProbeFile.txt --files sampleFiles_ill.txt --inDir ./fastqFiles_ill/ --outDir . --prefix ill_")

# nanopore dataset
system2("perl",
        args="./scripts/AmpliconReadCounter_modified.pl -p primerProbeFile.txt --files sampleFiles_ont.txt --inDir ./fastqFiles_ont/ --outDir . --prefix ont_")
```
...
at each locus to 10 reads with 100 iterations of downsampling:

```{r, eval=FALSE}
# illumina
alleleReads_ill_ds10x <- downsample_alleleReads(alleleReads_ill, 10, 100)

# nanopore
alleleReads_ont_ds10x <- downsample_alleleReads(alleleReads_ont, 10, 100)
```

```{r, eval=FALSE, echo=FALSE}
# export downsampled allele reads
write.table(alleleReads_ill_ds10x, "ill_alleleReads_ds10x.txt", quote = FALSE, sep = "\t")

write.table(alleleReads_ont_ds10x, "ont_alleleReads_ds10x.txt", quote = FALSE, sep = "\t")
```

```{r, echo=FALSE}
# quick-load downsampled allele reads
## add check.names = F so "X" won't be added to colnames
alleleReads_ill_ds10x <- read.table("ill_alleleReads_ds10x.txt", header = T, check.names = F)

alleleReads_ont_ds10x <- read.table("ont_alleleReads_ds10x.txt", sep = "\t", header = T, check.names = F)
```
...
```{r, echo=FALSE}
# For html
head(alleleReads_ill_ds10x) %>%
  kbl(caption = "Downsampled Illumina reads") %>%
  kable_material(full_width = F) %>%
  row_spec(0, color = "black", background = "tomato") %>%
  scroll_box(width = "100%", height = "200px", fixed_thead = list(enabled = T))

head(alleleReads_ont_ds10x) %>%
...
# genotype: illumina
polyGenResults_ill <- polyGen(locusTable, alleleReads_ill_ds10x)

# genotype: nanopore
polyGenResults_ont <- polyGen(locusTable, alleleReads_ont_ds10x)
```

```{r, eval=FALSE}
# export genotypes
write.table(polyGenResults_ill, "ill_polyGenResults.txt", quote = FALSE, sep = "\t")

write.table(polyGenResults_ont, "ont_polyGenResults.txt", quote = FALSE, sep = "\t")
```
...
```{r, echo=FALSE}
# illumina
genos_ill <- read.table("ill_polyGenResults.txt", header = T, check.names = F)
...
# nanopore
genos_ont <- read.table("ont_polyGenResults.txt", header = T, check.names = F)
...
Comparisons show that **100% of genotypes match** for all 10 samples at
all 74 loci between Illumina and Nanopore datasets.
