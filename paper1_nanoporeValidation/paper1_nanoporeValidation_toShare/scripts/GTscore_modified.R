# NOTE: RV modified lines 618 & 619 to add an option for "\\[ATGC\\]"="N" to replace brackets in probe (option wasn't present in original script)
...
alignMatchedSeqs<-function(referenceSeqs=NULL,primerProbes,matchedReads,minReads=1,maxAlignedSeqs=100,type="primerProbe",saveDir=""){
  ...
  for (x in 1:dim(loci)[1]){
    ...
    if(max(uniqueLocusReads$Count>=minReads)){
      ...
      formatMSAcmd<-paste("perl ./03_tamRun3/04_genoAnalyses/formatMSAalignments.pl", dfOut, sep=" ")
      system(formatMSAcmd)
